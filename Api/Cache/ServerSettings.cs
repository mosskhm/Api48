        ...
        public static string GetServerSettings(string NodeName, ref List<LogLines> lines)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        lines = Add2Log(lines, " Error on GetServerSettings " + ex.ToString(), 100, lines[0].ControlerName);
                    }
                    fs.Close();
                }
            }
            catch (Exception ex)
            {
                ...
                {
                    lines = Add2Log(lines, " Error on GetServerSettings " + ex1.ToString(), 100, lines[0].ControlerName);
                }
                fs.Close();
            }
