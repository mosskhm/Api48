...
namespace Api.Cache
{
    public class USSD
    {
        ...
        public class USSDMenu
        {
            ...
            public string allowed_msisdns { get; set; }
        ...
        public static USSDMenu GetUSSDMenu(int ussd_id, string ussd_string, int action_id, USSDSession ussd_session, ref List<LogLines> lines)
        {
                    ...
                    if (HttpContext.Current.Application["USSDMenuList_expdate"] != null)
                    {
                        ...
                        else
                        {
                            ...
                            if (result_list != null)
                            {
                                ...
                                HttpContext.Current.Application["USSDMenuList_expdate"] = DateTime.Now.AddMinutes(15);
                            }
            ...
            // check if we have a valid menu list
            if (result_list == null || result_list.Count == 0) lines = Add2Log(lines, " !! ERORR - failed to get USSD MENU LIST from database", 100, "");
            else
            { 
                // set certian ussd strings to default
                switch (ussd_string)
                {
                    case "205*1":
                    case "205*2":
                ...
                // if we have a valid session, then include the current topic_id
                if (ussd_session == null)
                    result = result_list.Find(x => x.prev_action_id == action_id && x.ussd_id == ussd_id && x.ussd_string.Contains(ussd_string));
                else
                    result = result_list.Find(x => x.prev_action_id == action_id && x.ussd_id == ussd_id && x.ussd_string.Contains(ussd_string) && x.topic_id == ussd_session.topic_id);

                // log what we found
                if (result == null) lines = Add2Log(lines, $"GetUSSDMenu: step 1 - failed to find menu for: ussd_id={ussd_id}, action_id = {action_id}, ussd_string={ussd_string}", 100, "");
                else lines = Add2Log(lines, $"GetUSSDMenu: step 1 - processing ussd_id={ussd_id}, action_id = {action_id}, ussd_string={ussd_string}, result = [ name: {result.action_name}, topic: {result.topic_name}, next action: {result.action_id}, service_id: {result.service_id} ]", 100, "");
                ...
                //if (action_id == 131)
                //{
                //    if (ussd_string.ToLower() != "m")
                //    {
                //        string tmp_str = ussd_string.Substring(0,1);
                //        Int64 num1;
                //        bool res = Int64.TryParse(tmp_str, out num1);
                //        if (res == true)
                //        {
                //            lines = Add2Log(lines, " Ussd is numeric", 100, "");
                //            result = result_list.Find(x => x.prev_action_id == action_id && x.ussd_id == ussd_id && x.topic_id == ussd_session.topic_id && x.ussd_string.ToLower() == "bankcode");
                //            result.is_special = true;
                //        }
                //        else
                //        {
                ...
                if (action_id == 78 && ussd_string.Length > 2 && (ussd_string.Substring(0, 2) == "06" || ussd_string.Substring(0, 1) == "6"))
                {
                ...
                if (action_id == 172)
                {
                    result = result_list.Find(x => x.prev_action_id == action_id && x.ussd_id == ussd_id && x.topic_id == ussd_session.topic_id && x.ussd_string.ToLower() == "bankaccount");
                }

                if (action_id == 173)
                {
                    result = result_list.Find(x => x.prev_action_id == action_id && x.ussd_id == ussd_id && x.topic_id == ussd_session.topic_id && x.ussd_string.ToLower() == "closemenu");
                }
                ...
                if (result == null)
                {
                    lines = Add2Log(lines, $"GetUSSDMenu: step 2 - looking up action_id={action_id}, ussd_id={ussd_id}", 100, "");
                ...
                if (result == null)
                {
                    lines = Add2Log(lines, $"GetUSSDMenu: step 3 - looking up action_id={action_id}, ussd_id={ussd_id}", 100, "");
            ...
            if (result == null) lines = Add2Log(lines, "GetUSSDMenu: returning NULL!!!!", 100, "");
            return result;
        }
