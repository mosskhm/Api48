...
{
    public class iDoBetLNBPariMTNBenin
    {
        ...
        public static string GetFootSoldiersMSISDNMenu(string MSISDN, string ussd_string, out bool is_close, ServiceClass service, ref List<LogLines> lines)
        {
            ...
            if (footsoldiers != null)
            {
                ...
                if (fs != null)
                {
                    ...
                    string token_id1 = "";
                    int service_id = GetServiceByMSISDNPrefix("229" + ussd_string, service.service_id.ToString(), out token_id1, ref lines);
                    if (success && service_id > 0 && service.service_id == service_id)
                    {
                        IdoBetUser user = Api.CommonFuncations.iDoBetLNBPariMTNBenin.SearchUserNew("229" + ussd_string, ref lines);
                        int user_id = 0;
                        try
                        {
                            user_id = user.id;
        ...
        public static CreateUserResponse CreateUser(string user_msisdn, string user_fullname, FootSoldiers fs, ref List<LogLines> lines)
        {
            ...
            string token_id = Api.Cache.iDoBet.GetMTNBeninLNBPiDoBetUserTokenNew(0, ref lines);
            ...
            if (!String.IsNullOrEmpty(token_id))
            {
                ...
                string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetEndPointUrlMTNBeninLNB", ref lines);
                ...
                headers.Add(new Headers { key = "BrandId", value = "20" });
                ...
                headers.Add(new Headers { key = "Terminal", value = "s.lnbpari.com" });
        ...
        public static ExecuteOrderDetails GetExecuteOrderNew(DYATransactions dya_trans, out string postBody, out string response_body, ref List<LogLines> lines)
        {
            ...
            postBody = "";
            response_body = "";
            List<SavedGames> saved_games = DataLayer.DBQueries.GetiDoBetSavedGames(dya_trans.partner_transid, "_mtnbj", ref lines);
            string token_id = Cache.iDoBet.GetMTNBeninLNBPiDoBetUserTokenNew(0, ref lines);
            if (saved_games != null)
            {
                ...
                string request_execute_order_url = Cache.ServerSettings.GetServerSettings("iDoBetExecuteOrderUrlMTNBeninLNB", ref lines);
                ...
                try
                {
                    ...
                    if (json_response.barcode != null && json_response.barcode != "null" && json_response.isSuccessfull == true)
                    {
                        ...
                        List<Int64> saved_ids = GetUSSDSavedGamesID(dya_trans.partner_transid, "_mtnbj", ref lines);
                        if (saved_ids != null)
                        {
                            string mysaved_id = "";
                            ...
                            if (!String.IsNullOrEmpty(mysaved_id))
                            {
                                ...
                                DataLayer.DBQueries.ExecuteQuery("update ussd_saved_games_mtnbj set barcode = '" + barcode + "', `status` = 2 where id in (" + mysaved_id + ")", "DBConnectionString_104", ref lines);
                            }
        ...
        public static ExecuteOrderDetails PlaceBetRapidos(DYATransactions dya_trans, out string postBody, out string response_body, ref List<LogLines> lines)
        {
            ...
            List<SavedGames> saved_games = DataLayer.DBQueries.GetiDoBetSavedGames(dya_trans.partner_transid.Replace("Rapidos_", ""), "_mtnbj", ref lines);
            string token_id = Cache.iDoBet.GetMTNBeninLNBPiDoBetUserTokenNew(0, ref lines);
            if (saved_games != null)
            {
                amount = saved_games[0].amount.ToString();
                ...
                string request_execute_order_url = Cache.ServerSettings.GetServerSettings("iDoBetPlaceBetLottoMTNBeninLNB", ref lines);
                lines = Add2Log(lines, " Sending to url " + request_execute_order_url, 100, "ivr_subscribe");
                response_body = CommonFuncations.CallSoap.CallSoapRequest(request_execute_order_url, postBody, headers, 2, true, ref lines);
                lines = Add2Log(lines, " Response " + response_body, 100, "ivr_subscribe");
                try
                {
                    ...
                    if (json_response.isSuccessfull = true)
                    {
                        ...
                        lines = Add2Log(lines, " Json = " + postBody, 100, "ivr_subscribe");
                        request_execute_order_url = Cache.ServerSettings.GetServerSettings("iDoBetExecuteOrderLottoUrlMTNBeninLNB", ref lines);
                        lines = Add2Log(lines, " Sending to url " + request_execute_order_url, 100, "ivr_subscribe");
                        response_body = CommonFuncations.CallSoap.CallSoapRequest(request_execute_order_url, postBody, headers, 2, true, ref lines);
                        lines = Add2Log(lines, " Response " + response_body, 100, "ivr_subscribe");
                        try
                        {
                            ...
                            if (json_response.isSuccessfull = true)
                            {
                                ...
                                List<Int64> saved_ids = GetUSSDSavedGamesID(dya_trans.partner_transid.Replace("Rapidos_", ""), "_mtnbj", ref lines);
                                if (saved_ids != null)
                                {
                                    string mysaved_id = "";
                                    foreach (Int64 s in saved_ids)
        ...
        public static bool ValidateRapidosInput(string ussd_string, int action_id, out String[] strlist, ref List<LogLines> lines)
        {
            ...
            if (ussd_string.Contains(" "))
            {
                ...
                // using the method 
                strlist = ussd_string.Split(spearator, count,StringSplitOptions.RemoveEmptyEntries);
                int counter = 0;
        ...
        {
            string menu = "";
            if (user.isValid == false)      menu = "Option invalide";
            else if (refund_money == true)  menu = "La demande de retrait a abouti" + Environment.NewLine;
            else
            {
                menu = "La demande de retrait a échoué" + Environment.NewLine;
                if (amount_2_big) menu += "Montant d'argent est trop grand" + Environment.NewLine;
                menu += "Veuillez réessayer";

            }
            lines = Add2Log(lines, $"refund={refund_money}, w_result={w_result}, amount2big={amount_2_big}, menu = {menu}" , 100, "");
        ...
        public static string GetPayoutBarcodeTicket(USSDMenu ussd_menu, string MSISDN, ref List<LogLines> lines)
        {
            ...
            if (!user_has_momo)
            {
                ...
                menu = menu + "taper *880# pour ouvrir un compte MOMO" + Environment.NewLine + Environment.NewLine;
        ...
        public static bool DoPayOutNew(string time_stamp, string dya_id, string bar_code, ref List<LogLines> lines)
        {
            ...
            if (!String.IsNullOrEmpty(token_id))
            {
                ...
                headers.Add(new Headers { key = "ExtraData", value = "" });
                headers.Add(new Headers { key = "Token", value = token_id });
                headers.Add(new Headers { key = "Terminal", value = Api.Cache.ServerSettings.GetServerSettings("iDoBetTerminalSecurityCodeMTNBeninLNB", ref lines) });
                            ...
                            if (sub_agents != null)
                            {
                                ...
                                if (filtered_sub_agent != null)
                                {
                                    ...
                                    if (trans_id > 0)
                                    {
                                        ...
                                        trans_id = CollectSendMoney(token, "920767", "CREDIT", pos_trans.ToString(), pos_details.amount, ref lines);
                                        if (trans_id > 0)
                                        {
        ...
        public static string GetAmountForKironAgentMenu(string MSISDN, string ussd_string, string selected_subagent_name, out Int64 pos_trans_id, ref List<LogLines> lines)
        {
            string menu = "";
            pos_trans_id = 0;
            List<iDoBetAgents> agents = Cache.iDoBet.GetiDoBetAgents(ref lines);
        ...
        public static string GetKironAgentMainMenu(string MSISDN, int page_number, ref List<LogLines> lines)
        {
            string menu = "";
            List<iDoBetAgents> agents = Cache.iDoBet.GetiDoBetAgents(ref lines);
            if (agents != null)
            {
                ...
                if (agent != null)
                {
                    menu = "Bonjour " + agent.agent_name + Environment.NewLine + Environment.NewLine;
                    menu = menu + "Entrez le montant que vous souhaitez recharger" + Environment.NewLine;

                    menu = menu + Environment.NewLine + "M)";
                }
            }
            if (menu == "")
            {
        ...
        public static string GetDepositMoneyMenu(IdoBetUser user, USSDMenu ussd_menu, string MSISDN, ref List<LogLines> lines)
        {
            ...
            if (user.isValid == false)
            {
                ...
                {
                    menu = "Rejoigner LNBPari pour gagner d'intéressants bonus!" + Environment.NewLine;
                    menu = menu + @"Visiter notre site http://m.lnbpari.com" + Environment.NewLine + Environment.NewLine;
                    menu = menu + "M) Menu Principal ";
                }
                else
                {
                    menu = "Quelque chose est arrivé s'il vous plaît appelez à l'équipe de soutien " + Environment.NewLine + Environment.NewLine; //call center
                    menu = menu + "M) Menu Principal ";
                }
            ...
            else
            {
                ...
                if (user_has_momo)
                {
                    ...
                    menu = menu + "Maximum: 1,000,000 CFA" + Environment.NewLine + Environment.NewLine;
                    menu = menu + "M) Menu Principal ";
                }
                else
                {
            ...
            else
            {
                ...
                if (user_has_momo)
                {
                    if (Convert.ToInt32(amount) >= 200 && Convert.ToInt32(amount) <= 1000000)
                    {
                        ...
                        menu = menu + "Pour compléter votre pari, confirmez le paiement ou composez *880#, puis sélectionnez (8)" + Environment.NewLine;
                    }
                    else
                    {
                        menu = menu + "Entrer votre montant" + Environment.NewLine + Environment.NewLine;
        ...
        public static string GetWithdrawMoneyMenu(IdoBetUser user, ref List<LogLines> lines)
        {
            ...
            else
            {
                ...
                else menu = menu + "Votre solde est " + user.AvailableForWithdraw + " CFA" + Environment.NewLine;
        ...
        public static IdoBetUser SearchUserNew(string MSISDN, ref List<LogLines> lines)
        {
            ...
            string token_id = Cache.iDoBet.GetMTNBeninLNBPiDoBetUserTokenCheckTicket(0, ref lines);
            if (!String.IsNullOrEmpty(token_id))
            {
        ...
        public static string GetInvoice(string barcode, ref List<LogLines> lines)
        {
            ...
            if (!string.IsNullOrEmpty(token_id))
            {
                string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetGetOrderKeyUrlMTNBeninLNB", ref lines);
                lines = Add2Log(lines, " idobet_endpoint = " + idobet_endpoint, 100, "ivr_subscribe");
                ...
                if (!String.IsNullOrEmpty(response_body))
                {
                    ...
                    try
                    {
                        ...
                        foreach (var item in json_response.data.orders)
                        {
                            ...
                            tickets.Add(new Tickets { msisdn = msisdn, barcode = barcode, payout = payout, creation_time = creation_time, amount = amount, win_status = win_status, order_status = order_status, page_number = page_number, max_payout = maxpayout, total_selections = total_selections });
        ...
        {
            bool result = false;
            List<SavedGames> saved_games = DataLayer.DBQueries.GetiDoBetSavedGames(ussd_session.user_seesion_id, "_mtnbj", ref lines);
            if (saved_games != null)
            {
                ...
                string token_id = Cache.iDoBet.GetMTNBeninLNBPiDoBetUserTokenNew(0, ref lines);
                string terminal_security_token = Cache.ServerSettings.GetServerSettings("iDoBetTerminalSecurityCodeMTNBeninLNB", ref lines);
                ...
                rows.Add(new rows { amount = saved_games[0].amount.ToString(), selectionKeys = selection_keys });

                foreach (SavedGames r in saved_games)
                {

                    selections.Add(new selections { eventId = Convert.ToInt64(r.game_id), betTypeId = r.selected_bet_type_id, oddName = r.selected_odd_name, oddLine = r.selected_odd_line, oddPrice = r.selected_odd.ToString(), key = r.game_id });
                }
                ...
                headers.Add(new Headers { key = "ExtraData", value = "" });
                headers.Add(new Headers { key = "Token", value = token_id });
                string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetPlaceBetUrlMTNBeninLNB", ref lines);
                string response_body = CommonFuncations.CallSoap.CallSoapRequest(idobet_endpoint, postBody, headers, 2, true, ref lines);
                lines = Add2Log(lines, " Response " + response_body, 100, "ivr_subscribe");
                try
                {
                    ...
                    {
                        result = true;
                        List<Int64> saved_ids = GetUSSDSavedGamesID(ussd_session.user_seesion_id, "_mtnbj", ref lines);
                        if (saved_ids != null)
                        {
                            string mysaved_id = "";
                            foreach (Int64 s in saved_ids)
                            ...
                            {
                                mysaved_id = mysaved_id.Substring(0, mysaved_id.Length - 1);
                                DataLayer.DBQueries.ExecuteQuery("update ussd_saved_games_mtnbj set time_out_guid = '" + json_response.timeoutGuid + "', `status` = 1 where id in (" + mysaved_id + ")", "DBConnectionString_104", ref lines);
                            }
        ...
        public static string GetSoccerLeagueMenu(int sport_type, string ussd_string, int ussd_id, ref List<LogLines> lines, out int out_league_id)
        {
            out_league_id = 0;
            string menu = "";
            List<SportEvents> sports_events = Api.Cache.iDoBet.GetMTNBeninLNBPEventsFromCache(sport_type, ref lines);
            switch (sport_type)
            {
                case 31:
        ...
        public static List<SportEvents> GetEvents(int selected_league_id, int sport_type_id, ref List<LogLines> lines)
        {
            ...
            if (!String.IsNullOrEmpty(token_id))
            {
                string get_events_url = Cache.ServerSettings.GetServerSettings("iDoBetGetEventsUrlMTNBeninLNB", ref lines) + "sportIds=" + sport_type_id + "&userToken=" + token_id + "&LeagueIds=" + selected_league_id;
        ...
        public static List<SportEvents> GetEvents(int sport_type_id, ref List<LogLines> lines)
        {
            ...
            if (!String.IsNullOrEmpty(token_id))
            {
                string get_events_url = Cache.ServerSettings.GetServerSettings("iDoBetGetEventsUrlMTNBeninLNB", ref lines) + "sportIds=" + sport_type_id + "&userToken=" + token_id;
                lines = Add2Log(lines, " get_events_url = " + get_events_url, 100, "ivr_subscribe");
                string response = CallSoap.GetURL(get_events_url, ref lines);
                if (!String.IsNullOrEmpty(response))
                {
        ...
        public static string GetConfirmMenu(int sport_type_id, Int64 game_id, string ussd_string, int odd_page, out double selected_odd, out int selected_bet_type_id, out string selected_odd_name, out string selected_odd_line, ref List<LogLines> lines)
        {
            ...
            selected_odd_name = "";
            selected_odd_line = "";
            List<SportEvents> sports_events = Cache.iDoBet.GetMTNBeninLNBPEventsFromCache(sport_type_id, ref lines);
        ...
        public static string GetGameOddsMenu(int sport_type_id, int page_number, string ussd_string, Int64 game_id, int odd_page, out Int64 out_game_id, int selected_league_id, ref List<LogLines> lines)
        {
            ...
            {
                sports_events = Cache.iDoBet.GetMTNBeninLNBPEventsFromCache(sport_type_id, ref lines);
            }
            else
            {
                sports_events = Cache.iDoBet.GetMTNBeninLNBPEventsWithLeagueIDFromCache(selected_league_id, sport_type_id, ref lines);
            }
        ...
        public static string GetCloseBet(USSDSession ussd_session, string amount, ref List<LogLines> lines)
        {
            ...
            if (saved_games != null)
            {
                ...
                menu = menu + "Pour compléter votre pari, confirmez le paiement ou composez *880#, puis sélectionnez (8)." + Environment.NewLine;
                menu = menu + "Confirmez T&amp;C et +18 http://m.lnbpari.com/terms" + Environment.NewLine;
            }
            return menu;
        }
        ...
        {
            string menu = "Veuillez saisir un montant compris entre 200 CFA et 1000000 CFA" + Environment.NewLine + Environment.NewLine;
            menu = menu + "*) Retour";
            return menu;
        }
        ...
        {
            string menu = "Veuillez saisir un montant compris entre 100 CFA et " + max_bet + " GNF" + Environment.NewLine + Environment.NewLine;
            menu = menu + "*) Retour";
            return menu;
        }
        ...
        public static string GetEventsMenu(string ussd_string, int sport_type_id, int page_number, ref List<LogLines> lines, out int selected_league_id, int real_selected_league_id, USSDMenu ussd_menu)
        {
            ...
            else
            {
                ...
                sports_events = Cache.iDoBet.GetMTNBeninLNBPEventsWithLeagueIDFromCache((selected_league_id == 0 ? real_selected_league_id : selected_league_id), sport_type_id, ref lines);
        ...
        public static string GetTokenNew(ref List<LogLines> lines)
        {
            string token_id = "";
            string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetEndPointUrlMTNBeninLNB", ref lines);
            string iDoBetUserName = Cache.ServerSettings.GetServerSettings("iDoBetLoginUserNameMTNBeninLNB", ref lines);
            string iDoBetPassword = Cache.ServerSettings.GetServerSettings("iDoBetLoginPasswordMTNBeninLNB", ref lines);
            lines = Add2Log(lines, " idobet_endpoint = " + idobet_endpoint, 100, "ivr_subscribe");
        ...
        public static string GetTokenCheckTicketNew(ref List<LogLines> lines)
        {
            string token_id = "";
            string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetEndPointUrlMTNBeninLNB", ref lines);
            string iDoBetUserName = Cache.ServerSettings.GetServerSettings("iDoBetCheckTicketLoginUserNameMTNBeninLNB", ref lines);
            string iDoBetPassword = Cache.ServerSettings.GetServerSettings("iDoBetCheckTicketLoginPasswordMTNBeninLNB", ref lines);
            lines = Add2Log(lines, " idobet_endpoint = " + idobet_endpoint, 100, "ivr_subscribe");
