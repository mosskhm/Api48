...
{
    public class iDoBetOrangeGuinea
    {
        ...
        public static string CheckFreeBet(USSDSession ussd_session, string msisdn, out bool placebet, out USSDBonus ub, ref List<LogLines> lines)
        {
            ...
            placebet = false;
            ub = null;
            List<SavedGames> saved_games = DataLayer.DBQueries.GetiDoBetSavedGames(ussd_session.user_seesion_id, "_ogn", ref lines);
            if (saved_games != null)
            {
                List<USSDBonus> ussd_b = GetUSSDBonusPerService("732", ref lines);
                if (ussd_b != null)
                {
        ...
        public static List<Tickets> CheckTicketsByUserIDNew(string idobetuser_id, ref List<LogLines> lines)
        {
            ...
            string token_id = Cache.iDoBet.GetOrangeGuineaiDoBetUserTokenCheckTicket(0, ref lines);
            if (!string.IsNullOrEmpty(token_id))
            {
                string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetEndPointUrlOrangeGuinea", ref lines);
                ...
                headers.Add(new Headers { key = "BrandId", value = "21" });
        ...
        public static string GetInvoice(string barcode, ref List<LogLines> lines)
        {
            ...
            //string password = Cache.ServerSettings.GetServerSettings("iDoBetCheckTicketLoginPassword", ref lines);
            //string token_id = GetToken(username, password, ref lines);
            string token_id = Cache.iDoBet.GetOrangeGuineiDoBetUserTokenNew(0, ref lines);
            if (!string.IsNullOrEmpty(token_id))
            {
                string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetGetOrderKeyUrlOrangeGuinea", ref lines);
                lines = Add2Log(lines, " idobet_endpoint = " + idobet_endpoint, 100, "ivr_subscribe");
        ...
        public static string GetConfirmDusanLottoMenu(int action_id, string ussd_string, out Int64 game_id, ref List<LogLines> lines)
        {
            ...
            menu = menu + Environment.NewLine + "Veuillez saisir un montant compris entre 2000 GNF et 5000 GNF" + Environment.NewLine;
        ...
        {
            string menu = "Ensemble de nombres invalide. cliquez sur 8 pour revenir au menu precedent" + Environment.NewLine + Environment.NewLine;
            menu = menu + "*) Retour";
            return menu;
        }
        ...
        public static string GetConfirmRapidosMenu(int action_id, string ussd_string, out Int64 game_id, ref List<LogLines> lines)
        {
            ...
            menu = menu + ussd_string;
            menu = menu + Environment.NewLine + "Veuillez saisir un montant compris entre " + min_bet + " GNF et " + max_bet + " GNF:" + Environment.NewLine;
            menu = menu + Environment.NewLine + "*) Retour ";
            return menu;
        }
        ...
        public static bool StartDepositNew(IdoBetUser user, string dya_id, string time_stamp, double amount, string msisdn, out string postBody, out string response_body, ref List<LogLines> lines)
        {
            ...
            if (!String.IsNullOrEmpty(token_id))
            {
                ...
                headers.Add(new Headers { key = "ExtraData", value = "" });
                headers.Add(new Headers { key = "Token", value = token_id });
                headers.Add(new Headers { key = "Terminal", value = Api.Cache.ServerSettings.GetServerSettings("iDoBetTerminalSecurityCodeOrangeGuinea", ref lines) });
        ...
        {
            bool result = false;
            string token_id = Api.Cache.iDoBet.GetOrangeGuineiDoBetUserTokenNew(0, ref lines);
        ...
        public static ExecuteOrderDetails PlaceBetRapidos(DYATransactions dya_trans, out string postBody, out string response_body, ref List<LogLines> lines)
        {
            ...
            List<SavedGames> saved_games = DataLayer.DBQueries.GetiDoBetSavedGames(dya_trans.partner_transid.Replace("Rapidos_", ""), "_ogn", ref lines);
            string token_id = Cache.iDoBet.GetOrangeGuineiDoBetUserTokenNew(0, ref lines);
            if (saved_games != null)
            {
                ...
                string request_execute_order_url = Cache.ServerSettings.GetServerSettings("iDoBetPlaceBetLottoOrangeGuinea", ref lines);
                ...
                try
                {
                    ...
                    if (json_response.isSuccessfull = true)
                    {
                        ...
                        lines = Add2Log(lines, " Json = " + postBody, 100, "ivr_subscribe");
                        request_execute_order_url = Cache.ServerSettings.GetServerSettings("iDoBetExecuteOrderLottoUrlOrangeGuinea", ref lines);
                        lines = Add2Log(lines, " Sending to url " + request_execute_order_url, 100, "ivr_subscribe");
                        response_body = CommonFuncations.CallSoap.CallSoapRequest(request_execute_order_url, postBody, headers, 2, true, ref lines);
                        lines = Add2Log(lines, " Response " + response_body, 100, "ivr_subscribe");
                        try
                        {
                            ...
                            if (json_response.isSuccessfull = true)
                            {
                                ...
                                List<Int64> saved_ids = GetUSSDSavedGamesID(dya_trans.partner_transid.Replace("Rapidos_", ""), "_ogn", ref lines);
                                if (saved_ids != null)
                                {
                                    string mysaved_id = "";
                                    ...
                                    if (!String.IsNullOrEmpty(mysaved_id))
                                    {
                                        ...
                                        DataLayer.DBQueries.ExecuteQuery("update ussd_saved_games_ogn set barcode = '" + barcode + "', `status` = 2 where id in (" + mysaved_id + ")", "DBConnectionString_104", ref lines);
                                    }
        ...
        public static ExecuteOrderDetails GetExecuteOrderNew(DYATransactions dya_trans, out string postBody, out string response_body, ref List<LogLines> lines)
        {
            ...
            postBody = "";
            response_body = "";
            List<SavedGames> saved_games = DataLayer.DBQueries.GetiDoBetSavedGames(dya_trans.partner_transid, "_ogn", ref lines);
            string token_id = Cache.iDoBet.GetOrangeGuineiDoBetUserTokenNew(0, ref lines);
            if (saved_games != null)
            {
                if (saved_games[0].time_out_guid != "0" && !String.IsNullOrEmpty(saved_games[0].time_out_guid))
                {
                    amount = saved_games[0].amount.ToString();
                    ...
                    string request_execute_order_url = Cache.ServerSettings.GetServerSettings("iDoBetExecuteOrderUrlOrangeGuinea", ref lines);
                    lines = Add2Log(lines, " Sending to url " + request_execute_order_url, 100, "ivr_subscribe");
                    response_body = CommonFuncations.CallSoap.CallSoapRequest(request_execute_order_url, postBody, headers, 2, true, ref lines);
                    lines = Add2Log(lines, " Response " + response_body, 100, "ivr_subscribe");
                    try
                    {
                        ...
                        if (json_response.barcode != null && json_response.barcode != "null" && json_response.isSuccessfull == true)
                        {
                            ...
                            List<Int64> saved_ids = GetUSSDSavedGamesID(dya_trans.partner_transid, "_ogn", ref lines);
                            if (saved_ids != null)
                            {
                                string mysaved_id = "";
                                foreach (Int64 s in saved_ids)
        ...
        public static bool PlaceBet(USSDSession ussd_session, ref List<LogLines> lines)
        {
            ...
            if (saved_games != null)
            {
                ...
                string token_id = Cache.iDoBet.GetOrangeGuineiDoBetUserTokenNew(0, ref lines);
                string terminal_security_token = Cache.ServerSettings.GetServerSettings("iDoBetTerminalSecurityCodeOrangeGuinea", ref lines);
                ...
                string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetPlaceBetUrlOrangeGuinea", ref lines);
                string response_body = CommonFuncations.CallSoap.CallSoapRequest(idobet_endpoint, postBody, headers, 2, true, ref lines);
                lines = Add2Log(lines, " Response " + response_body, 100, "ivr_subscribe");
                try
                {
                    ...
                    {
                        result = true;
                        List<Int64> saved_ids = GetUSSDSavedGamesID(ussd_session.user_seesion_id, "_ogn", ref lines);
                        if (saved_ids != null)
                        {
                            string mysaved_id = "";
                            foreach (Int64 s in saved_ids)
                            ...
                            {
                                mysaved_id = mysaved_id.Substring(0, mysaved_id.Length - 1);
                                DataLayer.DBQueries.ExecuteQuery("update ussd_saved_games_ogn set time_out_guid = '" + json_response.timeoutGuid + "', `status` = 1 where id in (" + mysaved_id+")", "DBConnectionString_104", ref lines);
                            }
                            ...
                            foreach (var a in json_response.validationResult[0].orderBetValidationResults)
                            {
                                if (a.validationResult == 241)
                                {
                                    ...
                                    if (saved_ids != null)
                                    {
                                        foreach (Int64 s in saved_ids)
                                        {
                                            Int64 myevent_id = Api.DataLayer.DBQueries.SelectQueryReturnInt64("select game_id from ussd_saved_games_ogn u where u.id = " + s, "DBConnectionString_104", ref lines);
                                            if (event_id == myevent_id)
                                            {
                                                Api.DataLayer.DBQueries.ExecuteQuery("update ussd_saved_games_ogn set selected_odd = " + odd_price + " where id = " + s, "DBConnectionString_104", ref lines);
                                                break;
                                            }
                                        }
                                    }
        ...
        public static List<SportEvents> GetEvents(int selected_league_id, int sport_type_id, ref List<LogLines> lines)
        {
            ...
            if (!String.IsNullOrEmpty(token_id))
            {
                string get_events_url = Cache.ServerSettings.GetServerSettings("iDoBetGetEventsUrlOrangeGuinea", ref lines) + "sportIds=" + sport_type_id + "&userToken=" + token_id + "&LeagueIds=" + selected_league_id;
        ...
        public static List<SportEvents> GetEvents(int sport_type_id, ref List<LogLines> lines)
        {
            ...
            if (!String.IsNullOrEmpty(token_id))
            {
                string get_events_url = Cache.ServerSettings.GetServerSettings("iDoBetGetEventsUrlOrangeGuinea", ref lines) + "sportIds=" + sport_type_id + "&userToken=" + token_id;
                lines = Add2Log(lines, " get_events_url = " + get_events_url, 100, "ivr_subscribe");
                string response = CallSoap.GetURL(get_events_url, ref lines);
                if (!String.IsNullOrEmpty(response))
                {
                        ...
                        if (sub_agents != null)
                        {
                            ...
                            if (filtered_sub_agent != null)
                            {
                                ...
                                if (available_credit > 0)
                                {
                                    ...
                                    menu = menu + "maximum " + available_credit + " GNF" + Environment.NewLine;
                                    menu = menu + Environment.NewLine + "*) Retour";
                                }
                                else
                                {
                                    menu = "Vous ne pouvez pas collecter 0 GNF aupres d'" + filtered_sub_agent.subagent_username + Environment.NewLine;
                                    menu = menu + Environment.NewLine + "*) Retour";
                                }
                            }
                        }
        ...
        public static string AddFSMenu(string menu, string MSISDN, ref List<LogLines> lines)
        {
            ...
            if (footsoldiers != null)
            {
                ...
                if (fs != null)
                {
                    menu = menu + "88) Foot Soldiers" + Environment.NewLine;
                }
            }
            return menu;
        }

        public static string AddLottoMenu(string menu, string MSISDN, ref List<LogLines> lines)
        {
            //if (MSISDN == "224627007626" || MSISDN == "22462700762655" || MSISDN == "224622627631" || MSISDN == "224622065358" || MSISDN == "224666010610" || MSISDN == "224622068132" || MSISDN == "224620623861")
            //{
            //    menu = menu + "7) Loto" + Environment.NewLine;
            //}
            return menu;
        }
        ...
        public static string GetFootSoldiersMSISDNMenu(string MSISDN, string ussd_string, out bool is_close, ServiceClass service, ref List<LogLines> lines)
        {
            ...
            if (footsoldiers != null)
            {
                ...
                if (fs != null)
                {
                    ...
                    if (success && (ussd_string.Substring(0, 2) == "62" || ussd_string.Substring(0, 2) == "61"))
                    {
                        IdoBetUser user = Api.CommonFuncations.iDoBetOrangeGuinea.SearchUserNew("224" + ussd_string, ref lines);
                        int user_id = 0;
                        try
                        {
                            user_id = user.id;
        ...
        public static string GetDepositMoneyMenu(IdoBetUser user, USSDMenu ussd_menu, string MSISDN, ref List<LogLines> lines)
        {
            ...
            else
            {
                ...
                if (user_has_momo)
                {
                    ...
                    menu = menu + "Minimum: 2000 GNF" + Environment.NewLine;
                    menu = menu + "Maximum: 6,000,000 GNF" + Environment.NewLine + Environment.NewLine;
                    menu = menu + "M) Menu Principal ";
                }
                else
                {
            ...
            else
            {
                ...
                if (user_has_momo)
                {
                    if (Convert.ToInt32(amount) >= 2000 && Convert.ToInt32(amount) <= 6000000)
                    {
                        ...
                        menu = menu + "Vous etes sur le point de déposer" + Environment.NewLine;
                        menu = menu + "Montant:" + amount + Environment.NewLine + Environment.NewLine;
        ...
        public static string GetSoccerLeagueMenu(int sport_type, string ussd_string, int ussd_id, ref List<LogLines> lines, out int out_league_id)
        {
            out_league_id = 0;
            string menu = "";
            List<SportEvents> sports_events = Cache.iDoBet.GetOrangeGuineaEventsFromCache(sport_type, ref lines);
            switch (sport_type)
            {
                case 31:
        ...
        public static string GetConfirmMenu(int sport_type_id, Int64 game_id, string ussd_string, int odd_page, out double selected_odd, out int selected_bet_type_id, out string selected_odd_name, out string selected_odd_line, ref List<LogLines> lines, USSDSession ussd_session, string msisdn, ServiceClass service)
        {
            ...
            selected_odd_name = "";
            selected_odd_line = "";
            List<SportEvents> sports_events = Cache.iDoBet.GetOrangeGuineaEventsFromCache(sport_type_id, ref lines);
        ...
        public static string GetGameOddsMenu(int sport_type_id, int page_number, string ussd_string, Int64 game_id, int odd_page, out Int64 out_game_id, int selected_league_id, ref List<LogLines> lines)
        {
            ...
            {
                sports_events = Cache.iDoBet.GetOrangeGuineaEventsFromCache(sport_type_id, ref lines);
            }
            else
            {
                sports_events = Cache.iDoBet.GetOrangeGuineaEventsWithLeagueIDFromCache(selected_league_id, sport_type_id, ref lines);
            }
            ...
            if (sports_events != null && game_id == 0)
            {
                ...
                if (filtered_sports_events != null)
                {
                    ...
                    {
                        lines = Add2Log(lines, " Exception " + ex.ToString(), 100, "dep_bigcash");
                    }
            ...
            if (game != null)
            {
                ...
                string game_time = Convert.ToDateTime(game.game_time).AddHours(-1).ToString("MMMM", CultureInfo.CreateSpecificCulture("fr")).Replace("û", "u").Replace("é", "e") + " " + Convert.ToDateTime(game.game_time).AddHours(-1).ToString("dd") + ", " + Convert.ToDateTime(game.game_time).AddHours(-1).ToString("HH:mm");
        ...
        public static string GetConfirmRapidosBet(Int64 game_id, string amount, ref List<LogLines> lines)
        {
            string menu = "";
            int max_p = 0;
            switch (game_id)
            {
                case 35:
                case 36:
                    max_p = Convert.ToInt32(amount) * 4;
                    break;
                case 37:
                    max_p = Convert.ToInt32(amount) * 25000;
                    break;
            }
            menu = "Confirmer votre Rapidos Pari:" + Environment.NewLine;
            menu = menu + "Montant:" + amount + Environment.NewLine;
            menu = menu + "Gain Maximum: " + max_p + Environment.NewLine + Environment.NewLine;
            menu = menu + "1) Confirmer" + Environment.NewLine;
        ...
        public static string GetConfirmLottoBet(string amount, ref List<LogLines> lines)
        {
            string menu = "";
            menu = "Confirmer votre Loto Pari:" + Environment.NewLine;
            menu = menu + "Montant:" + amount + Environment.NewLine;
            menu = menu + "1) Confirmer" + Environment.NewLine;
            menu = menu + "2) Annuler &amp; Recommencer" + Environment.NewLine + Environment.NewLine;
            menu = menu + "*) Retour ";
        ...
        public static string GetCloseBet(USSDSession ussd_session, string amount, ref List<LogLines> lines)
        {
            ...
            menu = menu + "Confirmez T&amp;C et +18 http://m.yellowbet.com.gn" + Environment.NewLine;
            return menu;
        }

        public static string GetWrongPriceBetMeny(ref List<LogLines> lines)
        {
            string menu = "Montant incorrect, inroduisez 8 pour retourner et un montant entre 2000 GNF et 6000000 GNF" + Environment.NewLine + Environment.NewLine;
            menu = menu + "*) Retour";
            return menu;
        }
        ...
        {
            string menu = "Montant incorrect, inroduisez 8 pour retourner et un montant entre 1200 GNF et " + max_bet + " GNF" + Environment.NewLine + Environment.NewLine;
            menu = menu + "*) Retour";
            return menu;
        }
        ...
        public static string GetEventsMenu(string ussd_string, int sport_type_id, int page_number, ref List<LogLines> lines, out int selected_league_id, int real_selected_league_id, USSDMenu ussd_menu)
        {
            ...
            else
            {
                ...
                sports_events = Cache.iDoBet.GetOrangeGuineaEventsWithLeagueIDFromCache((selected_league_id == 0 ? real_selected_league_id : selected_league_id), sport_type_id, ref lines);
        ...
        public static string GetTokenNew(ref List<LogLines> lines)
        {
            string token_id = "";
            string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetEndPointUrlOrangeGuinea", ref lines);
            string iDoBetUserName = Cache.ServerSettings.GetServerSettings("iDoBetLoginUserNameOrangeGuinea", ref lines);
            string iDoBetPassword = Cache.ServerSettings.GetServerSettings("iDoBetLoginPasswordOrangeGuinea", ref lines);
            lines = Add2Log(lines, " idobet_endpoint = " + idobet_endpoint, 100, "ivr_subscribe");
        ...
        public static string GetTokenCheckTicketNew(ref List<LogLines> lines)
        {
            string token_id = "";
            string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetEndPointUrlOrangeGuinea", ref lines);
            string iDoBetUserName = Cache.ServerSettings.GetServerSettings("iDoBetCheckTicketLoginUserNameOrangeGuinea", ref lines);
            string iDoBetPassword = Cache.ServerSettings.GetServerSettings("iDoBetCheckTicketLoginPasswordOrangeGuinea", ref lines);
            lines = Add2Log(lines, " idobet_endpoint = " + idobet_endpoint, 100, "ivr_subscribe");
