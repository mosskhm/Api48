...
namespace Api.CommonFuncations
{
    public class B2TechGNMTN
    {
        ...
        public static List<Tickets> GetLastNewTickets(string winstatus_id, ref List<LogLines> lines)
        {
            ...
            {
                string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetEndPointUrlMTNGuinea142", ref lines);
        ...
        public static string GetInvoice(string barcode, ref List<LogLines> lines)
        {
            ...
            if (!string.IsNullOrEmpty(token_id))
            {
                string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetGetOrderKeyUrlMTNGuinea142", ref lines);
                lines = Add2Log(lines, " idobet_endpoint = " + idobet_endpoint, 100, "ivr_subscribe");
        ...
        {
            bool result = false;
            string token_id = Cache.iDoBet.GetMTNGNCOToken(0, ref lines);
            response_body = "";
            postBody = "";
            if (!String.IsNullOrEmpty(token_id))
            {
        ...
        public static string StartWithdrawNew(IdoBetUser user, string msisdn, double amount, out string postBody, out string response_body, ref List<LogLines> lines)
        {
            ...
            postBody = "";
            response_body = "";
            string token_id = Cache.iDoBet.GetMTNGNPOToken(0, ref lines);
            if (!String.IsNullOrEmpty(token_id))
            {
        ...
        {
            bool result = false;
            string token_id = Api.Cache.iDoBet.GetMTNGNPOToken(0, ref lines);
            ...
            if (!String.IsNullOrEmpty(token_id))
            {
                ...
                headers.Add(new Headers { key = "ExtraData", value = "" });
                headers.Add(new Headers { key = "Token", value = token_id });
                headers.Add(new Headers { key = "Terminal", value = Api.Cache.ServerSettings.GetServerSettings("iDoBetTerminalSecurityCodeMTNGuinea142PO", ref lines) });
        ...
        public static ExecuteOrderDetails PlaceBetRapidos(DYATransactions dya_trans, out string postBody, out string response_body, ref List<LogLines> lines)
        {
            ...
            List<SavedGames> saved_games = DataLayer.DBQueries.GetiDoBetSavedGames(dya_trans.partner_transid.Replace("Rapidos_", ""), "_mgn", ref lines);
            string token_id = Cache.iDoBet.GetMTNGuineiDoBetUserTokenNew(0, ref lines);
            if (saved_games != null)
            {
                ...
                try
                {
                    ...
                    if (json_response.isSuccessfull = true)
                    {
                        ...
                        lines = Add2Log(lines, " Json = " + postBody, 100, "ivr_subscribe");
                        request_execute_order_url = Cache.ServerSettings.GetServerSettings("iDoBetExecuteOrderLottoUrlMTNGuinea142", ref lines);
                        lines = Add2Log(lines, " Sending to url " + request_execute_order_url, 100, "ivr_subscribe");
                        response_body = CommonFuncations.CallSoap.CallSoapRequest(request_execute_order_url, postBody, headers, 2, true, ref lines);
                        lines = Add2Log(lines, " Response " + response_body, 100, "ivr_subscribe");
                        try
                        {
                            ...
                            if (json_response.isSuccessfull = true)
                            {
                                ...
                                List<Int64> saved_ids = GetUSSDSavedGamesID(dya_trans.partner_transid.Replace("Rapidos_", ""), "_mgn", ref lines);
                                if (saved_ids != null)
                                {
                                    string mysaved_id = "";
                                    ...
                                    if (!String.IsNullOrEmpty(mysaved_id))
                                    {
                                        ...
                                        DataLayer.DBQueries.ExecuteQuery("update ussd_saved_games_mgn set barcode = '" + barcode + "', `status` = 2 where id in (" + mysaved_id + ")", "DBConnectionString_104", ref lines);
                                    }
        ...
        public static ExecuteOrderDetails GetExecuteOrderNew(DYATransactions dya_trans, out string postBody, out string response_body, ref List<LogLines> lines)
        {
            ...
            postBody = "";
            response_body = "";
            List<SavedGames> saved_games = DataLayer.DBQueries.GetiDoBetSavedGames(dya_trans.partner_transid, "_mgn", ref lines);
            string token_id = Cache.iDoBet.GetMTNGNCOToken(0, ref lines);
            string terminal_security_token = Cache.ServerSettings.GetServerSettings("iDoBetTerminalSecurityCodeMTNGuinea142CO", ref lines);
            if (saved_games != null)
            {
                amount = saved_games[0].amount.ToString();
                ...
                string request_execute_order_url = Cache.ServerSettings.GetServerSettings("iDoBetExecuteOrderUrlMTNGuinea142", ref lines);
                lines = Add2Log(lines, " Sending to url " + request_execute_order_url, 100, "ivr_subscribe");
                response_body = CommonFuncations.CallSoap.CallSoapRequest(request_execute_order_url, postBody, headers, 2, true, ref lines);
                lines = Add2Log(lines, " Response " + response_body, 100, "ivr_subscribe");
                try
                {
                    ...
                    if (json_response.barcode != null && json_response.barcode != "null" && json_response.isSuccessfull == true)
                    {
                        ...
                        List<Int64> saved_ids = GetUSSDSavedGamesID(dya_trans.partner_transid, "_mgn", ref lines);
                        if (saved_ids != null)
                        {
                            string mysaved_id = "";
                            foreach (Int64 s in saved_ids)
        ...
        {
            bool result = false;
            List<SavedGames> saved_games = DataLayer.DBQueries.GetiDoBetSavedGames(ussd_session.user_seesion_id, "_mgn", ref lines);
            if (saved_games != null)
            {
                ...
                string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetPlaceBetUrlMTNGuinea142", ref lines);
                string response_body = CommonFuncations.CallSoap.CallSoapRequest(idobet_endpoint, postBody, headers, 2, true, ref lines);
                lines = Add2Log(lines, " Response " + response_body, 100, "ivr_subscribe");
                try
                {
                    ...
                    {
                        result = true;
                        List<Int64> saved_ids = GetUSSDSavedGamesID(ussd_session.user_seesion_id, "_mgn", ref lines);
                        if (saved_ids != null)
                        {
                            string mysaved_id = "";
                            foreach (Int64 s in saved_ids)
                            ...
                            {
                                mysaved_id = mysaved_id.Substring(0, mysaved_id.Length - 1);
                                DataLayer.DBQueries.ExecuteQuery("update ussd_saved_games_mgn set time_out_guid = '" + json_response.timeoutGuid + "', `status` = 1 where id in (" + mysaved_id + ")", "DBConnectionString_104", ref lines);
                            }
                            ...
                            foreach (var a in json_response.validationResult[0].orderBetValidationResults)
                            {
                                if (a.validationResult == 241)
                                {
                                    ...
                                    if (saved_ids != null)
                                    {
                                        foreach (Int64 s in saved_ids)
                                        {
                                            Int64 myevent_id = Api.DataLayer.DBQueries.SelectQueryReturnInt64("select game_id from ussd_saved_games_mgn u where u.id = " + s, "DBConnectionString_104", ref lines);
                                            if (event_id == myevent_id)
                                            {
                                                Api.DataLayer.DBQueries.ExecuteQuery("update ussd_saved_games_mgn set selected_odd = " + odd_price + " where id = " + s, "DBConnectionString_104", ref lines);
                                                break;
                                            }
                                        }
                                    }
        ...
        public static List<SportEvents> GetEvents(int selected_league_id, int sport_type_id, ref List<LogLines> lines)
        {
            ...
            //{
                ...
                string get_events_url = Cache.ServerSettings.GetServerSettings("iDoBetGetEventsUrlMTNGuinea142", ref lines) + "sportTypeIds=" + sport_type_id + "&leagueIds=" + selected_league_id + "&toDate=" + mydate + "T00:00:00.000Z";
        ...
        public static List<SportEvents> GetEvents(int sport_type_id, ref List<LogLines> lines)
        {
            ...
            //{
            string mydate = DateTime.Now.AddDays(2).ToString("yyyy-MM-dd");
            string get_events_url = Cache.ServerSettings.GetServerSettings("iDoBetGetEventsUrlMTNGuinea142", ref lines) + "sportTypeIds=" + sport_type_id + "&toDate=" + mydate + "T00:00:00.000Z";
            lines = Add2Log(lines, " get_events_url = " + get_events_url, 100, "ivr_subscribe");
                string response = CallSoap.GetURL(get_events_url, ref lines);
                if (!String.IsNullOrEmpty(response))
                {
