...
namespace Api.CommonFuncations
{
    public class iDoBetMTNNG
    {
        ...
        public static List<Tickets> GetLastNewTickets(string winstatus_id, ref List<LogLines> lines)
        {
            ...
            string token_id = Cache.iDoBet.GetMTNCongoiDoBetUserTokenCheckTicket(0, ref lines);
            if (!string.IsNullOrEmpty(token_id))
            {
                string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetEndPointUrlMTNCongo", ref lines);
                ...
                headers.Add(new Headers { key = "BrandId", value = "22" });
        ...
        public static string GetInvoice(string barcode, ref List<LogLines> lines)
        {
            ...
            //string password = Cache.ServerSettings.GetServerSettings("iDoBetCheckTicketLoginPassword", ref lines);
            //string token_id = GetToken(username, password, ref lines);
            //string token_id = Cache.iDoBet.GetMTNCongoiDoBetUserTokenNew(0, ref lines);
            string token_id = Cache.iDoBet.GetMTNNGiDoBetUserTokenNew(0, ref lines);
            if (!string.IsNullOrEmpty(token_id))
            {
                string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetGetOrderKeyUrlMTNCongo", ref lines);
                lines = Add2Log(lines, " idobet_endpoint = " + idobet_endpoint, 100, "ivr_subscribe");
                    ...
                    try
                    {
                        if (json_response.data.ToString() != "[]")
                        {
                            ...
                            foreach (var item in json_response.data)
                            {
                                ...
                                available_for_withdra = Math.Round(available_for_withdra, 2);
            ...
            if (!String.IsNullOrEmpty(token_id))
            {
                ...
                if (!String.IsNullOrEmpty(response_body))
                {
                    ...
                    try
                    {
                        ...
                        //"IPin":3854,"IsValid":true,"IsLocked":false,"IsFullRegistrationDetails":true,"CurrencyCode":"NGN","AvailableForWithdraw":0.0,
        ...
        public static bool ValidateDusanLottoInput(string ussd_string, int action_id, out String[] strlist, ref List<LogLines> lines)
        {
            ...
            if (ussd_string.Contains(" "))
            {
                ...
                // using the method 
                strlist = ussd_string.Split(spearator,
                       StringSplitOptions.RemoveEmptyEntries);
                int counter = 0;
                foreach (string s in strlist)
                {
                ...
                if (count != counter)
                {
                    ...
                    {
                        if(counter >= count && counter <= max_number)
                        {
                            result = true;
                        }
                    }
        ...
        {
            string menu = "You have entered Invalid set of numbers. click * to return to the previous menu";
            return menu;
        }
        ...
        public static string GetConfirmRapidosMenu(int action_id, string ussd_string, out Int64 game_id, ref List<LogLines> lines)
        {
            string menu = "";
            game_id = 0;
            menu = "Confirm:" + Environment.NewLine;
            string min_bet = "1200";
            string max_bet = "";
            switch (action_id)
            {
            ...
            menu = menu + Environment.NewLine + "Please enter an amount between " + min_bet + " NGN and " + max_bet + " NGN:" + Environment.NewLine;
        ...
        public static string GetConfirmLottoMenu(int action_id, string ussd_string, out Int64 game_id, ref List<LogLines> lines)
        {
            ...
            menu = "Confirm:" + Environment.NewLine;
            menu = menu + ussd_string;
            menu = menu + Environment.NewLine + "Lotto Ticket cost is 2000 NGN" + Environment.NewLine;
            menu = menu + Environment.NewLine + "1) Confirm";
            menu = menu + Environment.NewLine + "*) Back ";
            return menu;
        }
        ...
        public static string GetConfirmDusanLottoMenu(int action_id, string ussd_string, out Int64 game_id, ref List<LogLines> lines)
        {
            ...
            menu = "Confirm " + game_name + ":" + Environment.NewLine;
            menu = menu + ussd_string;
            menu = menu + Environment.NewLine + "Please enter an amount between 63 NGN and 625 NGN" + Environment.NewLine;
            menu = menu + Environment.NewLine + "Commission: 20%" + Environment.NewLine;
            menu = menu + Environment.NewLine + "Enter amount:" + Environment.NewLine + Environment.NewLine;
            menu = menu + Environment.NewLine + "*) Back ";
            return menu;
        }
        ...
        {
            bool result = false;
            //string token_id = Api.Cache.iDoBet.GetMTNCongoiDoBetUserTokenNew(0, ref lines);
            string token_id = Cache.iDoBet.GetMTNNGiDoBetUserTokenNew(0, ref lines);
            ...
            if (!String.IsNullOrEmpty(token_id))
            {
                ...
                headers.Add(new Headers { key = "ExtraData", value = "" });
                headers.Add(new Headers { key = "Token", value = token_id });
                headers.Add(new Headers { key = "Terminal", value = Api.Cache.ServerSettings.GetServerSettings("iDoBetTerminalSecurityCodeMTNCongo", ref lines) });
        ...
        public static ExecuteOrderDetails PlaceBetRapidos(DYATransactions dya_trans, out string postBody, out string response_body, ref List<LogLines> lines)
        {
            ...
            if (saved_games != null)
            {
                ...
                string request_execute_order_url = Cache.ServerSettings.GetServerSettings("iDoBetPlaceBetLottoMTNCongo", ref lines);
                ...
                try
                {
                    ...
                    if (json_response.isSuccessfull = true)
                    {
                        ...
                        lines = Add2Log(lines, " Json = " + postBody, 100, "ivr_subscribe");
                        request_execute_order_url = Cache.ServerSettings.GetServerSettings("iDoBetExecuteOrderLottoUrlMTNCongo", ref lines);
                        lines = Add2Log(lines, " Sending to url " + request_execute_order_url, 100, "ivr_subscribe");
                        response_body = CommonFuncations.CallSoap.CallSoapRequest(request_execute_order_url, postBody, headers, 2, true, ref lines);
                        lines = Add2Log(lines, " Response " + response_body, 100, "ivr_subscribe");
                        try
                            ...
                            if (json_response.isSuccessfull = true)
                            {
                                ...
                                if (saved_ids != null)
                                {
                                    ...
                                    if (!String.IsNullOrEmpty(mysaved_id))
                                    {
                                        ...
                                        DataLayer.DBQueries.ExecuteQuery("update ussd_saved_games set barcode = '" + barcode + "', `status` = 2 where id in (" + mysaved_id + ")", ref lines);
                                    }
        ...
        public static ExecuteOrderDetails GetExecuteOrderNew(DYATransactions dya_trans, out string postBody, out string response_body, ref List<LogLines> lines)
        {
            ...
            if (saved_games != null)
            {
                ...
                string request_execute_order_url = Cache.ServerSettings.GetServerSettings("iDoBetExecuteOrderUrlMTNCongo", ref lines);
                lines = Add2Log(lines, " Sending to url " + request_execute_order_url, 100, "ivr_subscribe");
                response_body = CommonFuncations.CallSoap.CallSoapRequest(request_execute_order_url, postBody, headers, 2, true, ref lines);
                lines = Add2Log(lines, " Response " + response_body, 100, "ivr_subscribe");
                try
        ...
        public static bool PlaceBet(USSDSession ussd_session, ref List<LogLines> lines)
        {
            ...
            if (saved_games != null)
            {
                ...
                //string token_id = Cache.iDoBet.GetMTNCongoiDoBetUserTokenNew(0, ref lines);
                string token_id = Cache.iDoBet.GetMTNNGiDoBetUserTokenNew(0, ref lines);
                string terminal_security_token = Cache.ServerSettings.GetServerSettings("iDoBetTerminalSecurityCodeMTNNG", ref lines);
                ...
                string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetPlaceBetUrlMTNCongo", ref lines);
                string response_body = CommonFuncations.CallSoap.CallSoapRequest(idobet_endpoint, postBody, headers, 2, true, ref lines);
                lines = Add2Log(lines, " Response " + response_body, 100, "ivr_subscribe");
                try
                {
                    ...
                    if (json_response.timeoutGuid != null)
                    {
                        ...
                        if (saved_ids != null)
                        {
                            ...
                            {
                                mysaved_id = mysaved_id.Substring(0, mysaved_id.Length - 1);
                                DataLayer.DBQueries.ExecuteQuery("update ussd_saved_games set time_out_guid = '" + json_response.timeoutGuid + "', `status` = 1 where id in (" + mysaved_id + ")", ref lines);
                            }
                            ...
                            foreach (var a in json_response.validationResult[0].orderBetValidationResults)
                            {
                                if (a.validationResult == 241)
                                {
                                    ...
                                    if (saved_ids != null)
                                    {
                                        foreach (Int64 s in saved_ids)
                                        {
                                            Int64 myevent_id = Api.DataLayer.DBQueries.SelectQueryReturnInt64("select game_id from ussd_saved_games u where u.id = " + s, ref lines);
                                            if (event_id == myevent_id)
                                            {
                                                Api.DataLayer.DBQueries.ExecuteQuery("update ussd_saved_games set selected_odd = " + odd_price + " where id = " + s, ref lines);
                                                break;
                                            }
                                        }
                                    }
        ...
        public static List<SportEvents> GetEvents(int selected_league_id, int sport_type_id, ref List<LogLines> lines)
        {
            ...
            if (!String.IsNullOrEmpty(token_id))
            {
                string get_events_url = Cache.ServerSettings.GetServerSettings("iDoBetGetEventsUrlMTNCongo", ref lines) + "sportIds=" + sport_type_id + "&userToken=" + token_id + "&LeagueIds=" + selected_league_id;
        ...
        public static List<SportEvents> GetEvents(int sport_type_id, ref List<LogLines> lines)
        {
            ...
            if (!String.IsNullOrEmpty(token_id))
            {
                string get_events_url = Cache.ServerSettings.GetServerSettings("iDoBetGetEventsUrlMTNCongo", ref lines) + "sportIds=" + sport_type_id + "&userToken=" + token_id;
                lines = Add2Log(lines, " get_events_url = " + get_events_url, 100, "ivr_subscribe");
                string response = CallSoap.GetURL(get_events_url, ref lines);
                if (!String.IsNullOrEmpty(response))
                {
        ...
        public static string GetStartWithdrawMoneyMenu(IdoBetUser user, bool refund_money, bool w_result, bool amount_2_big, ref List<LogLines> lines)
        {
            ...
            else
            {
                ...
                {
                    menu = "The withdrawal request was successful" + Environment.NewLine;
                }
                else
                {

                    menu = "The withdrawal request has failed" + Environment.NewLine;
                    if (amount_2_big)
                    {
                        menu = menu + "Amount is too big" + Environment.NewLine;
                    }
                    menu = menu + "Please try again.";
                }
            }
                        ...
                        if (sub_agents != null)
                        {
                            ...
                            if (filtered_sub_agent != null)
                            {
                                ...
                                if (available_credit > 0)
                                {
                                    ...
                                    menu = menu + "Enter amount de l'argent" + Environment.NewLine;
                                    menu = menu + "maximum " + available_credit + " NGN" + Environment.NewLine;
                                    menu = menu + Environment.NewLine + "*) Back ";
                                }
                                else
                                {
                                    menu = "Vous ne pouvez pas collecter 0 NGN aupres d'" + filtered_sub_agent.subagent_username + Environment.NewLine;
                                    menu = menu + Environment.NewLine + "*) Back ";
                                }
                            }
                        }
        ...
        public static CreateUserResponse CreateUser(string user_msisdn, string user_fullname, FootSoldiers fs, ref List<LogLines> lines)
        {
            ...
            if (!String.IsNullOrEmpty(token_id))
            {
                ...
                headers.Add(new Headers { key = "ExtraData", value = "" });
                headers.Add(new Headers { key = "Token", value = token_id });
                headers.Add(new Headers { key = "Terminal", value = "yellowbet.com.gn" });
        ...
        public static string GetFootSoldiersMSISDNMenu(string MSISDN, string ussd_string, out bool is_close, ServiceClass service, ref List<LogLines> lines)
        {
            ...
            if (footsoldiers != null)
            {
                ...
                if (fs != null)
                {
                    ...
                    {
                        IdoBetUser user = Api.CommonFuncations.iDoBetMTNGuinea.SearchUserNew("224" + ussd_string, ref lines);
                        int user_id = 0;
                        try
                        {
                            user_id = user.id;
        ...
        public static string GetDepositMoneyMenu(IdoBetUser user, USSDMenu ussd_menu, string MSISDN, ref List<LogLines> lines)
        {
            ...
            if (user.isValid == false)
            {
                if (String.IsNullOrEmpty(user.firstName))
                {
                    ...
                    menu = menu + @"Visiter notre site http://m.yellowbet.com.gn" + Environment.NewLine + Environment.NewLine;
                    menu = menu + "M) Main Menu ";
                }
                else
                {
            ...
            else
            {
                ...
                if (user_has_momo)
                {
                    ...
                    menu = menu + "Minimum: 2000 NGN" + Environment.NewLine;
                    menu = menu + "Maximum: 6,000,000 NGN" + Environment.NewLine + Environment.NewLine;
                    menu = menu + "M) Main Menu ";
                }
                else
                {
        ...
        public static string GetStartDepositMoneyMenu(ServiceClass service, IdoBetUser user, USSDMenu ussd_menu, string MSISDN, string amount, out DYAReceiveMoneyRequest momo_request, ref List<LogLines> lines)
        {
            ...
            else
            {
                ...
                if (user_has_momo)
                {
                    if (Convert.ToInt32(amount) >= 2000 && Convert.ToInt32(amount) <= 6000000)
                    {
                        ...
                        menu = menu + "To complete your bet, please approve the payment";
                    }
                    else
                    {
                        menu = menu + "Entrer votre montant" + Environment.NewLine + Environment.NewLine;
        ...
        public static string GetWithdrawMoneyMenu(IdoBetUser user, ref List<LogLines> lines)
        {
            ...
            {
                menu = "Bonjour " + user.firstName + " " + user.lastName + Environment.NewLine + Environment.NewLine;
                menu = menu + "Votre solde est " + user.AvailableForWithdraw + " NGN" + Environment.NewLine;
                if (user.AvailableForWithdraw > 0)
                {

                    menu = menu + "Enter amount a retirer" + Environment.NewLine + Environment.NewLine;
                    menu = menu + "M) Main Menu ";
                }
                else
                {
        ...
        public static string GetCheckTicketByBarcodeMenu(string msisdn, string bar_codeid, out string paid_amount, ref List<LogLines> lines)
        {
            ...
            if (tickets != null)
            {
                ...
                if (filtered_ticket != null)
                {
                    ...
                    if (display_withdraw_option == true)
                    {
                        ...
                        menu = menu + Environment.NewLine + "1) Withdraw Money";
                    }
                    menu = menu + Environment.NewLine + "*) Back ";
            ...
            else
            {
                ...
                {
                    menu = "Ticket status:" + Environment.NewLine;
                    menu = menu + "Barcode: " + bar_codeid + Environment.NewLine;
                    string status = "";
                    ...
                    menu = menu + "Status: " + status + Environment.NewLine;
                    menu = menu + "Winning amount: " + dusan_ticket.amount + Environment.NewLine + Environment.NewLine;
                    menu = menu + Environment.NewLine + "*) Back ";
        ...
        public static string GetSoccerLeagueMenu(int sport_type, string ussd_string, int ussd_id, ref List<LogLines> lines, out int out_league_id)
        {
            out_league_id = 0;
            string menu = "";
            List<SportEvents> sports_events = Cache.iDoBet.GetMTNCongoEventsFromCache(sport_type, ref lines);
            switch (sport_type)
            {
                case 31:
                    menu = "Lets bet on football:" + Environment.NewLine;
                    break;
                case 32:
                    menu = "Lets bet on basketball:" + Environment.NewLine;
                    break;
                case 35:
                    menu = "Lets bet on tennis:" + Environment.NewLine;
                    break;
            }
            int counter = 2;
            ...
            {
                menu = menu + "1) All Events (" + sports_events.Count() + ")" + Environment.NewLine;
        ...
        public static string GetConfirmMenu(int sport_type_id, Int64 game_id, string ussd_string, int odd_page, out double selected_odd, out int selected_bet_type_id, out string selected_odd_name, out string selected_odd_line, ref List<LogLines> lines, USSDSession ussd_session, string msisdn, ServiceClass service)
        {
            ...
            selected_odd_name = "";
            selected_odd_line = "";
            List<SportEvents> sports_events = Cache.iDoBet.GetMTNCongoEventsFromCache(sport_type_id, ref lines);
            ...
            if (game != null)
            {
                try
                {
                    ...
                    menu = menu + "2) Confirm" + Environment.NewLine;
        ...
        {
            string menu = "Bad Choice";
            placebet = false;
            ub = null;
            List<SavedGames> saved_games = DataLayer.DBQueries.GetiDoBetSavedGames(ussd_session.user_seesion_id, ref lines);
            if (saved_games != null)
            {
                List<USSDBonus> ussd_b = GetUSSDBonusPerService("956", ref lines);
                if (ussd_b != null)
                {
        ...
        public static string GetGameOddsMenu(int sport_type_id, int page_number, string ussd_string, Int64 game_id, int odd_page, out Int64 out_game_id, int selected_league_id, ref List<LogLines> lines)
        {
            ...
            {
                sports_events = Cache.iDoBet.GetMTNCongoEventsFromCache(sport_type_id, ref lines);
            }
            else
            {
                sports_events = Cache.iDoBet.GetMTNCongoEventsWithLeagueIDFromCache(selected_league_id, sport_type_id, ref lines);
            }
            ...
            if (game != null)
            {
                ...
                string game_time = Convert.ToDateTime(game.game_time).AddHours(-1).ToString("MMMM", CultureInfo.CreateSpecificCulture("en")) + " " + Convert.ToDateTime(game.game_time).AddHours(-1).ToString("dd") + ", " + Convert.ToDateTime(game.game_time).AddHours(-1).ToString("HH:mm");
        ...
        public static string GetConfirmBet(USSDSession ussd_session, string amount, ref List<LogLines> lines)
        {
            ...
            if (saved_games != null)
            {
                ...
                int ticket_price = Convert.ToInt32(Math.Round(Convert.ToInt32(amount) * 0.8, 0));
                menu = menu + "Odds: " + total_ratio + Environment.NewLine;
                menu = menu + "Amount:" + amount + " (" + ticket_price + ")" + Environment.NewLine;
                menu = menu + "Commission: 20%" + Environment.NewLine;
                double max_payout = Math.Round(total_ratio * Convert.ToDouble(ticket_price), 2);
                max_payout = (max_payout >= 5000000 ? 5000000 : max_payout);
                menu = menu + "Maximum Win: " + max_payout + Environment.NewLine + Environment.NewLine;
        ...
        public static string EnterBankAccount(ref List<LogLines> lines)
        {
            string menu = "";
            menu = "Enter your bank account ";
            return menu;
        }

        public static string EnterFullName(ref List<LogLines> lines)
        {
            string menu = "";
        ...
        public static string CloseWinSapaBankMenu(ref List<LogLines> lines)
        {
            string menu = "";
            menu = "Thank you" + Environment.NewLine;
            menu = menu + "Your details will be added in 1 business day ";
            return menu;
        }

        public static string GetBankCode(int bank_id, ref List<LogLines> lines)
        {
            string result = "0";
        ...
        public static string GetCloseBet(USSDSession ussd_session, string amount, ref List<LogLines> lines)
        {
            ...
            menu = menu + "To complete your bet, please approve the payment.";
            //menu = menu + "Confirmez T&amp;C and +18 http://m.yellowbet.com.gn" + Environment.NewLine;
            return menu;
        }
        ...
        public static string GetCloseBetDusan(USSDSession ussd_session, string amount, ref List<LogLines> lines)
        {
            ...
            menu = menu + "To complete your bet, please approve the payment.";
            //menu = menu + "Confirmez T&amp;C and +18 http://mobile.yellowbet.com.gn" + Environment.NewLine;
            return menu;
        }
        ...
        public static string GetUSSDBonusCloseBet(ExecuteOrderDetails barcode, ref List<LogLines> lines)
        {
            ...
            if (barcode != null)
            {
                ...
                {
                    menu = menu + "Your free bet was approved" + Environment.NewLine;
                }
        ...
        {
            string menu = "You need to enter an amount between 63 NGN and 10000 NGN" + Environment.NewLine;
            menu = menu + "You will be charged 20% commision on airtime" + Environment.NewLine;
            menu = menu + "Click * to go back" + Environment.NewLine + Environment.NewLine;
            menu = menu + "*) Back ";
            return menu;
        }
        ...
        {
            string menu = "You need to enter an amount between 50 NGN and "+ max_bet + " NGN" + Environment.NewLine;
            menu = menu + "You will be charged 20% commision on airtime" + Environment.NewLine;
            menu = menu + "Click * to go back" + Environment.NewLine + Environment.NewLine;
            menu = menu + "*) Back ";
            return menu;
        ...
        {
            string menu = "";
            menu = menu + "Choose your sport:" + Environment.NewLine + Environment.NewLine;
        ...
        public static string GetEventsMenu(string ussd_string, int sport_type_id, int page_number, ref List<LogLines> lines, out int selected_league_id, int real_selected_league_id, USSDMenu ussd_menu)
        {
            ...
            else
            {
                ...
                sports_events = Cache.iDoBet.GetMTNCongoEventsWithLeagueIDFromCache((selected_league_id == 0 ? real_selected_league_id : selected_league_id), sport_type_id, ref lines);
        ...
        public static string GetTokenNew(ref List<LogLines> lines)
        {
            string token_id = "";
            string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetEndPointUrlMTNCongo", ref lines);
            string iDoBetUserName = Cache.ServerSettings.GetServerSettings("iDoBetLoginUserNameMTNNG", ref lines);
            string iDoBetPassword = Cache.ServerSettings.GetServerSettings("iDoBetLoginPasswordMTNNG", ref lines);
            lines = Add2Log(lines, " idobet_endpoint = " + idobet_endpoint, 100, "ivr_subscribe");
        ...
        public static string GetTokenCheckTicketNew(ref List<LogLines> lines)
        {
            string token_id = "";
            string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetEndPointUrlMTNCongo", ref lines);
            string iDoBetUserName = Cache.ServerSettings.GetServerSettings("iDoBetCheckTicketLoginUserNameMTNCongo", ref lines);
            string iDoBetPassword = Cache.ServerSettings.GetServerSettings("iDoBetCheckTicketLoginPasswordMTNCongo", ref lines);
            lines = Add2Log(lines, " idobet_endpoint = " + idobet_endpoint, 100, "ivr_subscribe");
