...
namespace Api.CommonFuncations
{
    public class iDoBetMTNCongo
    {
        ...
        public static string GetCloseDusanLotto(ref List<LogLines> lines)
        {
            ...
            menu = menu + "Confirmez T&amp;C et +18 http://m.yellowbet.cg" + Environment.NewLine;
            return menu;
        }
        ...
        public static string CheckFreeBet(USSDSession ussd_session, string msisdn, out bool placebet, out USSDBonus ub, ref List<LogLines> lines)
        {
            ...
            if (saved_games != null)
            {
                List<USSDBonus> ussd_b = GetUSSDBonusPerService("733", ref lines);
                if (ussd_b != null)
                {
        ...
        public static string GetFootSoldiersMSISDNMenu(string MSISDN, string ussd_string, out bool is_close, ServiceClass service, ref List<LogLines> lines)
        {
            ...
            if (footsoldiers != null)
            {
                ...
                if (fs != null)
                {
                    ...
                    if (success && ussd_string.Substring(0, 2) == "06")
                    {
                        IdoBetUser user = Api.CommonFuncations.iDoBetMTNCongo.SearchUserNew("242" + ussd_string, ref lines);
                        int user_id = 0;
                        try
                        {
                            user_id = user.id;
        ...
        public static CreateUserResponse CreateUser(string user_msisdn, string user_fullname, FootSoldiers fs, ref List<LogLines> lines)
        {
            ...
            if (!String.IsNullOrEmpty(token_id))
            {
                ...
                CreatreUserRequest request = new CreatreUserRequest()
                {
                    ...
                    userType = 2,
                    userName = user_msisdn.Substring(4),
                    email = "",
                    firstName = fname,
                    lastName = lname,
                    phone = user_msisdn,
            ...
            if (!String.IsNullOrEmpty(token_id))
            {
                ...
                if (!String.IsNullOrEmpty(response_body))
                {
                    ...
                    try
                    {
                        ...
                        {
                            string mail_body = "<p><b>DoPayout MTN Congo has failed for</b><br> <b>Barcode:</b> " + bar_code + "<br><br> <b>Request:</b> " + postBody + "<br><br> <b>Response:</b> " + response_body + "<br></p>";
                            string mail_subject = "Warning from DoPayout MTN Congo - " + bar_code;
                            string emails = Api.Cache.ServerSettings.GetServerSettings("iDoBetEmailRecipients", ref lines);
                            string sender_email = Api.Cache.ServerSettings.GetServerSettings("iDoBetSenderEmail", ref lines);
                            string sender_name = Api.Cache.ServerSettings.GetServerSettings("iDoBetSenderName", ref lines);
                            string sender_assword = Api.Cache.ServerSettings.GetServerSettings("iDoBetSenderPassword", ref lines);
                            ...
                            if (sub_agents != null)
                            {
                                ...
                                if (filtered_sub_agent != null)
                                {
                                    ...
                                    if (trans_id > 0)
                                    {
                                        ...
                                        trans_id = CollectSendMoney(token, "84222", "CREDIT", pos_trans.ToString(), pos_details.amount, ref lines);
                                        if (trans_id > 0)
                                        {
                        ...
                        if (json_response.data.ToString() != "[]")
                        {
                            ...
                            foreach (var item in json_response.data)
                            {
                                ...
                                user = new IdoBetUser()
                                {
                                    ...
                                    id = item.ID,
                                    availableCredit = item.AvailableCredit,
                                    AvailableForWithdraw = Math.Round(available_for_withdra,2),
                                    isLocked = item.IsLocked,
                                    Email = email,
                                    msisdn = item.Phone,
                                    RegistrationDate = reg_date
        ...
        public static ExecuteOrderDetails GetExecuteOrderNew(DYATransactions dya_trans, out string postBody, out string response_body, ref List<LogLines> lines)
        {
            ...
            if (saved_games != null)
            {
                try
                {
                    ...
                    lines = Add2Log(lines, " total_ratio = " + total_ratio, 100, "ivr_subscribe");
                    ...
                    if (json_response.barcode != null && json_response.barcode != "null" && json_response.isSuccessfull == true)
                    {
                        ...
                        if (saved_ids != null)
                        {
                            ...
                            //foreach (Int64 s in saved_ids)
                            //{
                            //    DataLayer.DBQueries.ExecuteQuery("update ussd_saved_games_cg set barcode = '" + barcode + "', `status` = 2 where id = " + s, ref lines);
                            //}
                        }
                    ...
                    else
                    {
                        ...
                        if (saved_ids != null)
                        {
                            ...
                            {
                                mysaved_id = mysaved_id.Substring(0, mysaved_id.Length - 1);
                                DataLayer.DBQueries.ExecuteQuery("delete from ussd_saved_games_cg where id in (" + mysaved_id + ")", "DBConnectionString_104", ref lines);
                            }

                            //foreach (Int64 s in saved_ids)
                            //{
                            //    DataLayer.DBQueries.ExecuteQuery("delete from ussd_saved_games_cg where id = " + s, ref lines);
                            //}
                        }
                        DataLayer.DBQueries.ExecuteQuery("insert into idobet_ids (dya_id, idobet_id) values(" + dya_trans.dya_trans + "," + idobet_transid + ")", ref lines);
                    }
            ...
            {
                lines = Add2Log(lines, " saved_games were not found " , 100, "ivr_subscribe");
            }
        ...
        public static bool PlaceBet(USSDSession ussd_session, ref List<LogLines> lines)
        {
            ...
            if (saved_games != null)
            {
                ...
                string token_id = Cache.iDoBet.GetMTNCongoiDoBetUserTokenNew(0, ref lines);
                string terminal_security_token = Cache.ServerSettings.GetServerSettings("iDoBetTerminalSecurityCodeMTNCongo", ref lines);
                ...
                try
                {
                    ...
                    if (json_response.timeoutGuid != null)
                    {
                        ...
                        if (saved_ids != null)
                        {
                            ...
                            //foreach (Int64 s in saved_ids)
                            //{
                            //    DataLayer.DBQueries.ExecuteQuery("update ussd_saved_games_cg set time_out_guid = '" + json_response.timeoutGuid + "', `status` = 1 where id = " + s, ref lines);
                            //}
                        }

                        //DataLayer.DBQueries.ExecuteQuery("update ussd_saved_games_cg set time_out_guid = '" + json_response.timeoutGuid + "', `status` = 1 where user_session_id = '" + ussd_session.user_seesion_id + "'", ref lines);
                    }
                    else
                    {
                        try
        ...
        public static string GetPayoutBarcodeTicket(USSDMenu ussd_menu, string MSISDN, ref List<LogLines> lines)
        {
            ...
            if (!user_has_momo)
            {
                menu = menu + "Si vous n'avez pas de compte MTN MOMO," + Environment.NewLine;
                menu = menu + "taper *105# pour ouvrir un compte MTN MOMO" + Environment.NewLine + Environment.NewLine;
                    ...
                    if (token != null)
                    {
                        ...
                        if (sub_agents != null)
                        {
                            ...
                            if (filtered_sub_agent != null)
                            {
                                ...
                                if (pos_trans_id > 0)
                                {
                                    menu = "Veuillez approuver la transaction MTN MOMO";
                                }
                                else
                                {
                                    menu = "Error 55";
            ...
            else
            {
                ...
                if (user_has_momo)
                {
                    if (Convert.ToInt32(amount) >= 200 && Convert.ToInt32(amount) <= 1000000)
                    {
                        ...
                        menu = menu + "Pour completer votre pari, confirmez le paiement ou composez le *105# " + Environment.NewLine + " ";
                    }
                    else
                    {
                        menu = menu + "Entrer votre montant" + Environment.NewLine + Environment.NewLine;
        ...
        {
            string menu = "Veuillez saisir un montant compris entre 200 CFA et " + max_bet + " CFA" + Environment.NewLine + Environment.NewLine;
            menu = menu + "*) Retour ";
            return menu;
        }
        ...
        public static string GetCloseBet(USSDSession ussd_session, string amount, ref List<LogLines> lines)
        {
            ...
            if (saved_games != null)
            {
                ...
                menu = menu + "Pour completer votre pari, confirmez le paiement ou composez le *105#" + Environment.NewLine;
                menu = menu + "Confirmez T&amp;C et +18 http://m.yellowbet.cg/terms " + Environment.NewLine + " ";
            }
            return menu;
        }
        ...
        public static string GetTokenNew(ref List<LogLines> lines)
        {
            string token_id = "";
            string idobet_endpoint = Cache.ServerSettings.GetServerSettings("iDoBetEndPointUrlMTNCongo", ref lines);
            string iDoBetUserName = Cache.ServerSettings.GetServerSettings("iDoBetLoginUserNameMTNCongo", ref lines);
            string iDoBetPassword = Cache.ServerSettings.GetServerSettings("iDoBetLoginPasswordMTNCongo", ref lines);
            lines = Add2Log(lines, " idobet_endpoint = " + idobet_endpoint, 100, "ivr_subscribe");
