...
namespace Api.CommonFuncations
{
    public class Cameroon
    {
        public static bool SendSMSKannel(string msisdn, string short_code, string text, bool is_flash, ref List<LogLines> lines)
        {
            ...
            short_code = (short_code == "8519" ? "YellowBet" : short_code);
            string url = Cache.ServerSettings.GetServerSettings("CameroonKannelURL", ref lines) + "from=" + short_code + "&to=" + msisdn + "&text=" + System.Uri.EscapeDataString(text) + "&dlr-mask=24" + (is_flash == true ? "&mclass=0" : "");
