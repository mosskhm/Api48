    ...
    public class SendSMS
    {
        ...
        public static SendSMSResponse DoSMS(SendSMSRequest RequestBody)
        {
            ...
            if (ValidateRequest1(RequestBody, ref lines))
            {
                ...
                {
                    ret = DoSMS(RequestBody, 1);
                }
                else
                {
                    DLValidateSMS result = new DLValidateSMS();
                    //if (service.operator_id == 8 || service.operator_id == 12 || service.is_ondemand == true)
                    //{
                        result = DBQueries.ValidateSMSRequest1(RequestBody, ref lines);
                    //}
                    //else
                    //{
                    ...
                    if ((result != null) && (result.RetCode == 1000))
                    {
                        ...
                        switch (result.operator_id)
                        {
                            ...
                            case 18:
                                ...
                                break;
                            case 15:
                                bool ogsend_sms = Orange.SendSMS(RequestBody, service, ref lines);
                                if (ogsend_sms)
                                {
                                    lines = Add2Log(lines, "SMS was sent", 100, "SendSMS");
                                    ret = new SendSMSResponse()
                                ...
                                break;
                            case 7:
                                bool gnsend_sms = Guinea.SendSMSKannel(RequestBody.MSISDN.ToString(), result.SMSMTCode.ToString(), RequestBody.Text, is_flash, ref lines);
                                if (gnsend_sms)
                                {
                                    lines = Add2Log(lines, "SMS was sent", 100, "SendSMS");
                                    ret = new SendSMSResponse()
                            ...
                            case 22:
                                ...
                                break;
                            case 2: //Nigeria NJ
                                bool send_sms_ps = PISIMobile.SendSMS(RequestBody, service, ref lines);
                                if (send_sms_ps)
                                {
                                    lines = Add2Log(lines, "SMS was sent", 100, "SendSMS");
                                    ret = new SendSMSResponse()
                            ...
                            default: //mtn sdp
                                ...
                                {
                                    bool send_smsmadapi = MADAPI.SendSMS(RequestBody, service, ref lines);
                                    if (send_smsmadapi)
                                    {
                                        lines = Add2Log(lines, "SMS was sent", 100, "SendSMS");
                                        ret = new SendSMSResponse()
