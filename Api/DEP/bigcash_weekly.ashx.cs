...
{
    /// <summary>
    /// Summary description for bigcash_weekly
    /// </summary>
    public class bigcash_weekly : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            lines = Add2Log(lines, "*****************************", 100, "dep_bigcashweekly");
            lines = Add2Log(lines, "IP = " + context.Request.ServerVariables["REMOTE_ADDR"], 100, "dep_bigcashweekly");
            lines = Add2Log(lines, "UA = " + context.Request.ServerVariables["HTTP_USER_AGENT"], 100, "dep_bigcashweekly");
            lines = Add2Log(lines, "REFERER = " + context.Request.ServerVariables["HTTP_REFERER"], 100, "dep_bigcashweekly");

            foreach (String key in context.Request.QueryString.AllKeys)
            {
                lines = Add2Log(lines, "Key: " + key + " Value: " + context.Request.QueryString[key], 100, "dep_bigcashweekly");
            }
            ...
            string redirect_url = "http://doi.dep.mtn.co.za/service/9807?ext_ref=123456776";
            if (!string.IsNullOrEmpty(context.Request.QueryString["msisdn"]) && !string.IsNullOrEmpty(context.Request.QueryString["result"]) && !string.IsNullOrEmpty(context.Request.QueryString["subscription_id"]))
            {
                ...
                if (REFERER.Contains("doi.mtndep.co.za"))
                {
                    ...
                    if (String.IsNullOrEmpty(real_subid))
                    {
                        ...
                        real_subid = Api.DataLayer.DBQueries.ExecuteQueryReturnInt64("insert into subscribers (msisdn, service_id, subscription_date, state_id) values(" + msisdn + ",1013,'" + mytime + "',1)", ref lines).ToString();
                ...
                redirect_url = "https://ydotgames-ydvas.netlify.app/Redirect?cli=" + msisdn + "&sid=1013";
            }
            ...
            if (context.Request.Cookies["cli"] != null)
            {
                ...
                {
                    redirect_url = "https://ydotgames-ydvas.netlify.app/Redirect?cli=" + context.Request.Cookies["cli"].Value + "&sid=1013";
                }
            }

            lines = Add2Log(lines, " Redirecting to  = " + redirect_url, 100, "dep_bigcashweekly");
