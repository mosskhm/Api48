...
{
    /// <summary>
    /// Summary description for sports7
    /// </summary>
    public class sports7 : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            lines = Add2Log(lines, "*****************************", 100, "dep_sports7");
            lines = Add2Log(lines, "IP = " + context.Request.ServerVariables["REMOTE_ADDR"], 100, "dep_sports7");
            lines = Add2Log(lines, "UA = " + context.Request.ServerVariables["HTTP_USER_AGENT"], 100, "dep_sports7");
            lines = Add2Log(lines, "REFERER = " + context.Request.ServerVariables["HTTP_REFERER"], 100, "dep_sports7");

            foreach (String key in context.Request.QueryString.AllKeys)
            {
                lines = Add2Log(lines, "Key: " + key + " Value: " + context.Request.QueryString[key], 100, "dep_sports7");
            }
            ...
            string redirect_url = "http://doi.dep.mtn.co.za/service/7908?ext_ref=123456776";
            if (!string.IsNullOrEmpty(context.Request.QueryString["msisdn"]) && !string.IsNullOrEmpty(context.Request.QueryString["result"]) && !string.IsNullOrEmpty(context.Request.QueryString["subscription_id"]))
            {
                ...
                if (REFERER.Contains("doi.mtndep.co.za"))
                {
                    ...
                    if (String.IsNullOrEmpty(real_subid))
                    {
                        ...
                        real_subid = Api.DataLayer.DBQueries.ExecuteQueryReturnInt64("insert into subscribers (msisdn, service_id, subscription_date, state_id) values(" + msisdn + ",781,'" + mytime + "',1)", ref lines).ToString();
                ...
                redirect_url = "https://sports7.netlify.app/redirect.html?cli=" + msisdn + "&sid=781";
            }
            ...
            if (context.Request.Cookies["cli"] != null)
            {
                ...
                {
                    redirect_url = "https://sports7.netlify.app/redirect.html?cli=" + context.Request.Cookies["cli"].Value + "&sid=781";
                }
            }

            lines = Add2Log(lines, " Redirecting to  = " + redirect_url, 100, "dep_sports7");
