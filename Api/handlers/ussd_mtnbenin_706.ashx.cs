...
{
    /// <summary>
    /// Summary description for ussd_mtnbenin_706
    /// </summary>
    public class ussd_mtnbenin_706 : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            lines = Add2Log(lines, "*****************************", Convert.ToInt32(ConfigurationManager.AppSettings["log_level"]), "ussd_mtnbenin706");
            lines = Add2Log(lines, "Incomming XML = " + xml, 100, "ussd_mo");
            lines = Add2Log(lines, "IP = " + context.Request.ServerVariables["REMOTE_ADDR"], 100, "ussd_mtnbenin706");
            lines = Add2Log(lines, "UA = " + context.Request.ServerVariables["HTTP_USER_AGENT"], 100, "ussd_mtnbenin706");
            lines = Add2Log(lines, "REFERER = " + context.Request.ServerVariables["HTTP_REFERER"], 100, "ussd_mtnbenin706");
            ...
            if (!String.IsNullOrEmpty(xml))
            {
                ...
                lines = Add2Log(lines, " MSISDN = " + MSISDN, 100, "ussd_mtnbenin706");

                ussdString = ProcessXML.GetXMLNode(xml, "subscriberInput", ref lines);
                lines = Add2Log(lines, " ussdString = " + ussdString, 100, "ussd_mtnbenin706");

                time_stamp = ProcessXML.GetXMLNode(xml, "dateFormat", ref lines);
                lines = Add2Log(lines, " time_stamp = " + time_stamp, 100, "ussd_mtnbenin706");
                                ...
                                case "1":
                                    ...
                                    if (momo_response1 != null)
                                    {
                                        if (momo_response1.ResultCode == 1010)
                                        {
                                            ...
                                            menu_2_display = "Yello! Pour terminer votre paiement, confirmez le paiement ou composez le *880#, puis sélectionnez (8).";
                                        }
                                        else
                                        {
                                    ...
                                    break;
                                case "2":
                                    menu_2_display = "Cher abonné, merci d'utiliser MTN Millionnaire";
                                    break;
                            }
                        }
                        else
            ...
            lines = Add2Log(lines, " is_close =  " + is_close, 100, "ussd_mtnbenin");
