...
{
    /// <summary>
    /// Summary description for ussd_mtnguinea
    /// </summary>
    public class ussd_mtnguinea : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            lines = Add2Log(lines, "*****************************",100, "ussd_mtnguinea");
            lines = Add2Log(lines, "Incomming XML = " + xml, 100, "ussd_mo");
            lines = Add2Log(lines, "IP = " + context.Request.ServerVariables["REMOTE_ADDR"], 100, "ussd_mtnguinea");
            lines = Add2Log(lines, "UA = " + context.Request.ServerVariables["HTTP_USER_AGENT"], 100, "ussd_mtnguinea");
            lines = Add2Log(lines, "REFERER = " + context.Request.ServerVariables["HTTP_REFERER"], 100, "ussd_mtnguinea");
            ...
            if (!String.IsNullOrEmpty(xml))
            {
                ...
                lines = Add2Log(lines, " MSISDN = " + MSISDN, 100, "ussd_mtnguinea");

                ussdString = ProcessXML.GetXMLNode(xml, "subscriberInput", ref lines);
                lines = Add2Log(lines, " ussdString = " + ussdString, 100, "ussd_mtnguinea");

                time_stamp = ProcessXML.GetXMLNode(xml, "dateFormat", ref lines);
                lines = Add2Log(lines, " time_stamp = " + time_stamp, 100, "ussd_mtnguinea");
                ...
                if (umc != null && !xml.Contains("cleanup"))
                {
                    ...
                    if (ussd_menu != null)
                    {
                        GoogleAnalytics.SendData2GoogleAnalytics("UA-172015213-3", "ussd", Base64.Reverse(MSISDN), context.Request.ServerVariables["REMOTE_ADDR"], "BJ", "pageview", "", "", "", "/" + ussd_menu.action_name, ref lines);
                        ...
                        if (ussd_menu.is_maintenance)
                        {
                            ...
                            {
                                ussd_soap = Api.CommonFuncations.USSD.MTNGuinea142USSDBehaviuer(service, ussdString, ServiceID, MSISDN, linkid, receiveCB, senderCB, serviceCode, ussd_menu, ussd_session, out momo_request, ref lines, out menu_2_display, out is_close, "");
                            }
                            else
                            {
                        ...
                        try
                        {
                            lines = Add2Log(lines, " menu_2_display = " + menu_2_display + ", Length = " + menu_2_display.Length, 100, "ivr_subscribe");
                        }
                        catch(Exception ex)
                        {

                        }
                        if (momo_request != null && String.IsNullOrEmpty(menu_2_display))
                        {
                            menu_2_display = "Merci. Veuillez confirmer votre paiement";
                        }
                ...
                if (is_close)
                {
                    lines = Add2Log(lines, " its close up time " + MSISDN, 100, "ussd_mtnguinea");
                    freeflowState = "FB";
                    USSDSession ussd_session = DataLayer.DBQueries.GetLastUSSDSession(MSISDN, umc.ussd_id, ref lines);
                ...
                {
                    lines = Add2Log(lines, " its clean up time " + MSISDN, 100, "ussd_mtnguinea");
                    menu_2_display = "";
                    freeflowState = "FB";
                    //USSDSession ussd_session = DataLayer.DBQueries.GetLastUSSDSession(MSISDN, umc.ussd_id, ref lines);
                    //if (ussd_session != null)
                    //{
                    //    Api.DataLayer.DBQueries.ExecuteQuery("update ussd_sessions set `status` = 1 where session_id = " + ussd_session.session_id, ref lines);
                    //}
                }
            }
            ...
            soap = soap + "</freeflow>";
            soap = soap + "</response>";
            lines = Add2Log(lines, "soap = " + soap, 100, "ussd_mtnguinea");
