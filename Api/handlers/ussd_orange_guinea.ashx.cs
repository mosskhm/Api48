...
{
    /// <summary>
    /// Summary description for ussd_orange_guinea
    /// </summary>
    public class ussd_orange_guinea : IHttpHandler
    {
        ...
        public static string BuildOrangeMenu(string msisdn, string menu_2_display, bool is_close, int action_id, USSDSession ussd_session, USSDMenu ussd_menu, ref List<LogLines> lines)
        {
            ...
            if (!String.IsNullOrEmpty(menu_2_display))
            {
                ...
                else
                {
                    ...
                    if (menu_2_display.Contains("Entrer le montant") || menu_2_display.Contains("Entrer votre montant")  || menu_2_display.Contains("Chaque numero est separe par un espace") || menu_2_display.Contains("Entrez le nom complet de l'utilisateur") || menu_2_display.Contains("Entrez le numero de telephone de l'utilisateur") || menu_2_display.Contains("Entrez le code a barres"))
                    {
                        form_type = "form";
                    }
                    ...
                    foreach (string s in strlist)
                    {
                        if (s.Contains(")"))
                        {
                            ...
                            if (option != "")
                            {
                                ...
                                string my_url = (HttpContext.Current.Request.ServerVariables["SERVER_PORT"] == "443" ? "https://" : "http://") + HttpContext.Current.Request.ServerVariables["SERVER_NAME"] + ":" + HttpContext.Current.Request.ServerVariables["SERVER_PORT"] + "/USSDEmulator/OrangeGuinea/" + option + "?ANI=" +msisdn + "&DNIS=" + option;
                                ...
                                if (text.ToLower() == "confirmer" && menu_2_display.Contains("Montant:"))
                                {
                                    ...
                                    d_type = (menu_2_display.Contains("Confirmer votre Loto Pari") == true ? 4 : d_type);
                                    string myservice_id = (d_type == 4 ? "888" : "732");


                                    Int64 ref_id = Api.DataLayer.DBQueries.ExecuteQueryReturnInt64("insert into orange_billing_requests (msisdn, date_time, amount, session_id, d_type, service_id) values("+msisdn+",now(),"+amount+",'"+ ussd_session.user_seesion_id+ "',"+ d_type + ","+ myservice_id + ")", "DBConnectionString_161", ref lines);
                                    if (ref_id > 0)
                                    {
                                        my_url = "http://10.173.84.177:8080/yellow-bet/orangeMoneyPayment?msisdn=" + msisdn + "&amount=" + amount + "&referenceNumbere=" + ref_id;
                                    }
                    ...
                    if (form_type == "form")
                    {
                        ...
                        string my_url = (HttpContext.Current.Request.ServerVariables["SERVER_PORT"] == "443" ? "https://" : "http://") + HttpContext.Current.Request.ServerVariables["SERVER_NAME"] + ":" + HttpContext.Current.Request.ServerVariables["SERVER_PORT"] + "/USSDEmulator/OrangeGuinea/O" + "?ANI=" + msisdn;
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            lines = Add2Log(lines, "Incomming XML = " + xml, 100, "ussd_orange_guinea");
            lines = Add2Log(lines, "IP = " + context.Request.ServerVariables["REMOTE_ADDR"], 100, "ussd_orange_guinea");
            lines = Add2Log(lines, "UA = " + context.Request.ServerVariables["HTTP_USER_AGENT"], 100, "ussd_orange_guinea");
            lines = Add2Log(lines, "REFERER = " + context.Request.ServerVariables["HTTP_REFERER"], 100, "ussd_orange_guinea");
            foreach (String key in context.Request.QueryString.AllKeys)
            {
                lines = Add2Log(lines, "Key: " + key + " Value: " + context.Request.QueryString[key], 100, "ussd_orange_guinea");
            }
