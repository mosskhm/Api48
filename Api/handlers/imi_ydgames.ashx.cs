...
{
    /// <summary>
    /// Summary description for imi_ydgames
    /// </summary>
    public class imi_ydgames : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            lines = Add2Log(lines, "*****************************", 100, "imi_ydgames");
            lines = Add2Log(lines, "IP = " + context.Request.ServerVariables["REMOTE_ADDR"], 100, "imi_ydgames");
            lines = Add2Log(lines, "UA = " + context.Request.ServerVariables["HTTP_USER_AGENT"], 100, "imi_ydgames");
            lines = Add2Log(lines, "REFERER = " + context.Request.ServerVariables["HTTP_REFERER"], 100, "imi_ydgames");
            string redirect_url = "";
            string secret_key = "grprXPlrZa";
            foreach (String key in context.Request.QueryString.AllKeys)
            {
                lines = Add2Log(lines, "Key: " + key + " Value: " + context.Request.QueryString[key], 100, "imi_ydgames");
            }
            ...
            {
                //check user
                ServiceClass service = GetServiceByServiceID(1, ref lines);
                    ...
                    if (res.ResultCode == 1000)
                    {
                        ...
                        if (res1 != null)
                        {
                            ...
                            if (res1.ResultCode == 1000)
                            {
                                ...
                                {
                                    //add user
                                    sub_id = DataLayer.DBQueries.InsertSub(m, "1", 0, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), "", "7", "IMI", ref lines);
                ...
                redirect_url = "http://gamesmtn.ydot.co/gamesmtn/index?cli=" + CommonFuncations.Base64.EncodeDecodeBase64(m, 1);
            ...
            else
            {
                ...
                lines = Add2Log(lines, " params are empty redirecting to http://mtnplay.co.za/portal/jqm.aspx?typxm=subscribecheckydgames", 100, "imi_ydgames");
                redirect_url = "http://mtnplay.co.za/portal/jqm.aspx?typxm=subscribecheckydgames";
            }
