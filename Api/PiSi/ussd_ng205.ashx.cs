...
namespace Api.PiSi
{
    ...
    public class ussd_ng205 : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            lines = Add2Log(lines, "*****************************", 100, "pisi_ussd_ng205");
            lines = Add2Log(lines, "Incomming XML = " + xml, 100, "pisi_ussd_ng205");
            lines = Add2Log(lines, "IP = " + context.Request.ServerVariables["REMOTE_ADDR"], 100, "pisi_ussd_ng205");
            lines = Add2Log(lines, "UA = " + context.Request.ServerVariables["HTTP_USER_AGENT"], 100, "pisi_ussd_ng205");
            lines = Add2Log(lines, "REFERER = " + context.Request.ServerVariables["HTTP_REFERER"], 100, "pisi_ussd_ng205");

            string messageType = "", serviceCode = "", MSISDN = "", traceUniqueID = "", msgType = "", ussdString = "", cu_id = "", abort_soap = "", sessionId = "";
            string ussd_soap = "";
            ...
            if (!String.IsNullOrEmpty(xml))
            {
                ...
                serviceCode = json_response.serviceCode;
                lines = Add2Log(lines, " serviceCode = " + serviceCode, 100, "ussd_mo");


                string spID = "205", ServiceID = "", linkid = "", receiveCB = "FFFFFFFF", senderCB = "186597739";

                USSDMainCode umc = GetUSSDMainCodeID(serviceCode, serviceCode, ref lines);
                ...
                if (service != null && ussdString != "")
                {
                    if (umc != null)
                    {
                        if (ussdString == "205*99")
                        {
                            umc = GetUSSDMainCodeID("20599", "20599", ref lines);
                        }
                        ...
                        {
                            ussd_session = DataLayer.DBQueries.GetLastUSSDSession(MSISDN, 18, ref lines);
                            if (ussd_session != null)
                            {
                                umc = GetUSSDMainCodeID("20599", "20599", ref lines);
                            }
                        ...
                        if (ussdString == "205*99" || umc.ussd_id == 18)
                        {
                            USSDMenu ussd_menu = null;//Api.CommonFuncations.USSD.GetSendCryptoMenu(umc.ussd_id, ussdString, action_id, ussd_session, MSISDN, sessionId, ref lines);
                            if (ussd_menu != null)
                            {
            ...
            if (!String.IsNullOrEmpty(menu_2_display))
            {
                ...
                response_soap = response_soap + "\"serviceCode\": \""+ serviceCode + "\",";
            ...
            if (momo_request != null)
            {
                ...
                string url = "https://api239.ydplatform.com:8443/api/DYAReceiveMoney";
                lines = Add2Log(lines, "url = " + url, 100, "ussd_mo");
                lines = Add2Log(lines, "postBody = " + postBody, 100, "ussd_mo");
                List<Headers> headers = new List<Headers>();
                try
                {
                    string body = CommonFuncations.CallSoap.CallSoapRequest(url, postBody, headers, 2, ref lines);
                    lines = Add2Log(lines, "Finished Sending momo request " + body, 100, "ussd_mo");
                }
                catch(Exception ex)
                {
                    lines = Add2Log(lines, "Exception  " + ex.ToString(), 100, "ussd_mo");
                }
