...
{
    /// <summary>
    /// Summary description for lnb_moov
    /// </summary>
    public class lnb_moov : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            service = GetServiceByServiceID(730, ref lines);
            ...
            if (umc != null && !String.IsNullOrEmpty(msisdn) && !String.IsNullOrEmpty(user_input))
            {
                ...
                if (ussd_menu != null)
                {
                    ...
                    if (ussd_menu.is_maintenance)
                    {
                        ...
                        {
                            ussd_soap = Api.CommonFuncations.USSD.LNBPariMoovUSSDBehaviuerV142(service, user_input, ServiceID, msisdn, linkid, receiveCB, senderCB, serviceCode, ussd_menu, ussd_session, out momo_request, ref lines, out menu_2_display, out is_close, "");
                        }
                        else
                        {
            ...
            else
            {
                ...
                xml = xml + "		<text>Bonjour et bienvenue sur LNBPari!</text>";
                xml = xml + "		<options>";
                xml = xml + "			<option choice=\"1\">Paris Sportifs</option>";
